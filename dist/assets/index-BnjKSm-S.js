...
[
  ...
  `;c+=n.map((o,d)=>{const u=o.priority>=4?"High":o.priority>=3?"Medium":"Low",l=o.priority>=4?"priority-high":o.priority>=3?"priority-med":"priority-low";return`
      <div class="focus-card card-enter" style="animation-delay: ${d*.1}s" data-subtask-id="${o.id}">
    ...
    `}).join(""),c+="</div>",t.innerHTML=c,t.querySelectorAll(".btn-start").forEach(o=>{o.addEventListener("click",d=>{const u=d.currentTarget.dataset.id,l=parseInt(d.currentTarget.dataset.minutes),p=n.find(v=>v.id===u);Tt(u,l,p?p.title:"Task")})}),t.querySelectorAll(".btn-done").forEach(o=>{o.addEventListener("click",async d=>{const u=d.currentTarget.dataset.id;d.currentTarget.closest(".focus-card").classList.add("card-complete"),setTimeout(async()=>{await q(u)?f("🎉 Task Complete!","All subtasks for this task are done!"):f("✅ Subtask Done!","Great work! Keep going!"),N()},500)})})}function Tt(t,n,s){kt(t,n,s,()=>{N()})}function St(){const t=document.getElementById("app-content");t.innerHTML=`
  ...
  `,tt(),{cleanup:()=>{}}}async function tt(){const t=document.getElementById("task-list-content");if(!t)return;const n=await L(),s=n.filter(r=>r.active);if(s.length===0){t.innerHTML=`
    ...
    `;for(const o of c){const d=await w(o.id),u=d.filter(m=>m.status==="done").length,l=d.length,p=l>0?Math.round(u/l*100):0;let v="";o.category==="periodic"&&o.intervalDays?v=`Every ${o.intervalDays} days`:o.completedDates.length>0&&(v=`Done ${o.completedDates.length} time${o.completedDates.length>1?"s":""}`),e+=`
    ...
    `),t.innerHTML=e,t.querySelectorAll(".btn-delete").forEach(r=>{r.addEventListener("click",async c=>{const o=c.currentTarget.dataset.taskId,d=c.currentTarget.closest(".task-item");confirm("Delete this task and all its subtasks?")&&(d.classList.add("card-exit"),setTimeout(async()=>{await _(o),f("🗑️ Deleted","Task removed"),tt()},300))})})}function It(){const t=document.getElementById("app-content");t.innerHTML=`
