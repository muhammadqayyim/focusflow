...
[
  ...
  `,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("timer-overlay-show"));let i=!1;const r=document.getElementById("timer-display"),c=document.getElementById("timer-progress-ring"),o=2*Math.PI*88,l=document.getElementById("btn-pause");function u(){r&&(r.textContent=Q(x));const y=1-x/N,v=o*y;c&&(c.style.strokeDashoffset=v)}function d(){i||(x--,u(),x<=0&&(clearInterval(T),T=null,p()))}function p(){Tt("⏰ Time's Up!",`Timer for "${n}" has ended.`),"vibrate"in navigator&&navigator.vibrate([200,100,200,100,200]),document.querySelector(".timer-controls").style.display="none",document.getElementById("timeup-panel").style.display="block"}function h(y){x+=y*60,N+=y*60,document.getElementById("timeup-panel").style.display="none",document.querySelector(".timer-controls").style.display="flex",u(),T||(T=setInterval(d,1e3)),f("⏱️ Time Added",`+${y} minutes added`)}function m(y){T&&(clearInterval(T),T=null),e.classList.remove("timer-overlay-show"),e.classList.add("timer-overlay-hide"),setTimeout(()=>{e.remove(),a&&a()},400)}T=setInterval(d,1e3),l.addEventListener("click",()=>{i=!i,l.innerHTML=i?'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polygon points="5 3 19 12 5 21 5 3"/></svg> Resume':'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> Pause',l.classList.toggle("btn-resume",i)}),document.getElementById("btn-finish-early").addEventListener("click",async()=>{await G(t),f("✅ Done!","Great focus session!"),m()}),document.getElementById("btn-done-final").addEventListener("click",async()=>{await G(t),f("🎉 Complete!","You crushed it!"),m()}),e.querySelectorAll(".btn-add-time[data-minutes]").forEach(y=>{y.addEventListener("click",()=>{h(parseInt(y.dataset.minutes))})});const w=document.getElementById("random-slider"),K=document.getElementById("random-value");w.addEventListener("input",()=>{K.textContent=`${w.value} min`}),document.getElementById("btn-random-add").addEventListener("click",()=>{const y=parseInt(w.value),v=Math.max(1,Math.floor(Math.random()*y)+1);h(v),f("🎲 Random Time",`Added ${v} random minutes!`)})}function Q(t){const s=Math.floor(t/60),n=t%60;return`${s.toString().padStart(2,"0")}:${n.toString().padStart(2,"0")}`}let F=null;function xt(){const t=document.getElementById("app-content");return t.innerHTML=`
    <div class="dashboard">
  ...
  `,X(),H(),document.getElementById("btn-refresh").addEventListener("click",async()=>{const s=document.getElementById("btn-refresh");s.disabled=!0;const n=s.querySelector("svg");n.style.animation="spin 1s linear infinite",await nt(),await H(),s.disabled=!1,n.style.animation=""}),F=setInterval(X,6e4),{cleanup:()=>{F&&clearInterval(F)}}}function X(){const t=document.getElementById("greeting");if(!t)return;const s=new Date().getHours();let n="Good evening";s>=5&&s<12?n="Good morning":s>=12&&s<17&&(n="Good afternoon");const a=["🔥","💪","⚡","🚀","✨","🎯","💫"],e=a[Math.floor(Math.random()*a.length)];t.textContent=`${n} ${e}`}async function H(){const t=document.getElementById("focus-cards");if(!t)return;const{subtasks:s,task:n}=await _();if(!n||s.length===0){t.innerHTML=`
  ...
  `,ot(),{cleanup:()=>{}}}async function ot(){const t=document.getElementById("task-list-content");if(!t)return;const s=await D(),n=s.filter(r=>r.active);if(n.length===0){t.innerHTML=`
    ...
    `;for(const o of c){const l=await S(o.id),u=l.filter(m=>m.status==="done").length,d=l.length,p=d>0?Math.round(u/d*100):0;let h="";o.category==="periodic"&&o.intervalDays?h=`Every ${o.intervalDays} days`:o.completedDates.length>0&&(h=`Done ${o.completedDates.length} time${o.completedDates.length>1?"s":""}`),e+=`
        <div class="task-item card-enter" data-task-id="${o.id}">
          <div class="task-item-content">
            <div class="task-item-title">${o.title}</div>
            ${h?`<div class="task-item-meta">${h}</div>`:""}
    ...
    `),t.innerHTML=e,t.querySelectorAll(".btn-delete").forEach(r=>{r.addEventListener("click",async c=>{const o=c.currentTarget.dataset.taskId,l=c.currentTarget.closest(".task-item");confirm("Delete this task and all its subtasks?")&&(l.classList.add("card-exit"),setTimeout(async()=>{await V(o),f("🗑️ Deleted","Task removed"),ot()},300))})})}function Dt(){const t=document.getElementById("app-content");t.innerHTML=`
  ...
  `;const s=document.getElementById("api-key-form"),n=document.getElementById("api-key");I("geminiApiKey").then(i=>{i&&(n.value=i)}),s.addEventListener("submit",async i=>{i.preventDefault();const r=n.value.trim();if(!r){await E("geminiApiKey",null),f("⚠️ Cleared","API Key removed");return}await E("geminiApiKey",r),f("✅ Saved!","Your API Key is securely stored.")});const a=document.getElementById("btn-test-api"),e=document.getElementById("api-test-results");return a.addEventListener("click",async()=>{var r;const i=n.value.trim();if(!i)return f("⚠️ Error","Please enter an API Key first.");a.disabled=!0,a.innerHTML="Testing...",e.style.display="block",e.innerHTML='<p class="loading-text">Fetching authorized models...</p>';try{const c=await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${i}`);if(!c.ok){const d=await c.json();throw new Error(((r=d.error)==null?void 0:r.message)||`HTTP ${c.status}`)}const l=(await c.json()).models||[];if(l.length===0){e.innerHTML='<p class="error-text">No models found for this key.</p>';return}const u=l.filter(d=>d.name.includes("gemini"));e.innerHTML=`
      ...
      `,f("✅ Connected",`Found ${u.length} Gemini models!`)}catch(c){console.error(c),e.innerHTML=`<p class="error-text">❌ Connection Failed: ${c.message}</p>`,f("❌ Failed","API Key check failed")}finally{a.disabled=!1,a.innerHTML="Test Connection"}}),{cleanup:()=>{}}}async function Bt(){const t=document.getElementById("app");t.innerHTML=`
    <div class="app-shell">
  ...
  `,await g(),await gt(),wt(),A("#dashboard",xt),A("#add",$t),A("#tasks",Lt),A("#settings",Dt);const s=document.querySelectorAll(".nav-btn");function n(){const a=window.location.hash||"#dashboard";s.forEach(e=>{e.classList.toggle("active",e.dataset.route===a)})}s.forEach(a=>{a.addEventListener("click",()=>{window.location.hash=a.dataset.route})}),window.addEventListener("hashchange",n),kt("#dashboard"),n()}Bt().catch(console.error);
