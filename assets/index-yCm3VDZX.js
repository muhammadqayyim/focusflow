(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))a(e);new MutationObserver(e=>{for(const i of e)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function n(e){const i={};return e.integrity&&(i.integrity=e.integrity),e.referrerPolicy&&(i.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?i.credentials="include":e.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(e){if(e.ep)return;e.ep=!0;const i=n(e);fetch(e.href,i)}})();const rt="focusflow",ct=1;let $=null;function g(){return new Promise((t,s)=>{if($)return t($);const n=indexedDB.open(rt,ct);n.onupgradeneeded=a=>{const e=a.target.result;if(e.objectStoreNames.contains("tasks")||e.createObjectStore("tasks",{keyPath:"id"}).createIndex("category","category",{unique:!1}),!e.objectStoreNames.contains("subtasks")){const i=e.createObjectStore("subtasks",{keyPath:"id"});i.createIndex("taskId","taskId",{unique:!1}),i.createIndex("status","status",{unique:!1})}e.objectStoreNames.contains("appState")||e.createObjectStore("appState",{keyPath:"key"})},n.onsuccess=a=>{$=a.target.result,t($)},n.onerror=a=>s(a.target.error)})}function b(t,s="readonly"){return $.transaction(t,s).objectStore(t)}function k(t){return new Promise((s,n)=>{t.onsuccess=()=>s(t.result),t.onerror=()=>n(t.error)})}async function R(t){return await g(),k(b("tasks","readwrite").put(t))}async function lt(t){return await g(),k(b("tasks").get(t))}async function D(){return await g(),k(b("tasks").getAll())}async function V(t){await g();const s=await S(t),n=b("subtasks","readwrite");for(const a of s)n.delete(a.id);return k(b("tasks","readwrite").delete(t))}async function O(t){return R(t)}async function L(t){return await g(),k(b("subtasks","readwrite").put(t))}async function dt(t){return await g(),k(b("subtasks").get(t))}async function S(t){await g();const s=b("subtasks").index("taskId");return k(s.getAll(t))}async function ut(t){return L(t)}async function A(t){return await g(),k(b("subtasks","readwrite").delete(t))}async function E(t){await g();const s=await k(b("appState").get(t));return s?s.value:null}async function I(t,s){return await g(),k(b("appState","readwrite").put({key:t,value:s}))}const j=Object.freeze(Object.defineProperty({__proto__:null,addSubtask:L,addTask:R,deleteSubtask:A,deleteTask:V,getAllTasks:D,getState:E,getSubtask:dt,getSubtasksByTask:S,getTask:lt,openDB:g,setState:I,updateSubtask:ut,updateTask:O},Symbol.toStringTag,{value:"Module"})),pt="modulepreload",mt=function(t){return"/focusflow/"+t},W={},q=function(s,n,a){let e=Promise.resolve();if(n&&n.length>0){let r=function(l){return Promise.all(l.map(u=>Promise.resolve(u).then(d=>({status:"fulfilled",value:d}),d=>({status:"rejected",reason:d}))))};document.getElementsByTagName("link");const c=document.querySelector("meta[property=csp-nonce]"),o=(c==null?void 0:c.nonce)||(c==null?void 0:c.getAttribute("nonce"));e=r(n.map(l=>{if(l=mt(l),l in W)return;W[l]=!0;const u=l.endsWith(".css"),d=u?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const p=document.createElement("link");if(p.rel=u?"stylesheet":pt,u||(p.as="script"),p.crossOrigin="",p.href=l,o&&p.setAttribute("nonce",o),document.head.appendChild(p),u)return new Promise((f,m)=>{p.addEventListener("load",f),p.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${l}`)))})}))}function i(r){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=r,window.dispatchEvent(c),!c.defaultPrevented)throw r}return e.then(r=>{for(const c of r||[])c.status==="rejected"&&i(c.reason);return s().catch(i)})};let vt=0;function Z(){return Date.now().toString(36)+"-"+(++vt).toString(36)+"-"+Math.random().toString(36).slice(2,7)}function ft(t,s,n=null){return{id:Z(),title:t.trim(),category:s,intervalDays:n,createdAt:Date.now(),completedDates:[],lastScheduledDate:null,active:!0}}function tt(t,s,n,a,e=0){return{id:Z(),taskId:t,title:s.trim(),estimatedMinutes:n,priority:a,order:e,status:"pending",completedAt:null,addedTimeMinutes:0}}const J={daily:t=>[{t:`Set up & prepare to start: ${t}`,min:5,p:3},{t:`Focus on the main work: ${t}`,min:20,p:5},{t:`Wrap up & quick review of: ${t}`,min:5,p:4}],periodic:t=>[{t:`Recall where you left off & review progress on: ${t}`,min:5,p:3},{t:`Do the core work session: ${t}`,min:20,p:5},{t:`Document progress & set reminders for next time: ${t}`,min:5,p:4}],oneoff:t=>[{t:`Research & figure out the approach for: ${t}`,min:10,p:3},{t:`Execute the main action: ${t}`,min:20,p:5},{t:`Verify completion & do any follow-ups for: ${t}`,min:10,p:4}],new:t=>[{t:`Understand requirements & ask clarifying questions about: ${t}`,min:10,p:3},{t:`Create first draft / initial attempt: ${t}`,min:20,p:5},{t:`Review your work & refine: ${t}`,min:10,p:4}]};function yt(t){return console.warn("Using fallback breakdown for:",t.title),(J[t.category]||J.daily)(t.title).map((a,e)=>tt(t.id,a.t,a.min,a.p,e))}async function C(t){var a;const s=await E("geminiApiKey");if(!s)return yt(t);const n=`
...
[
  ...
  `.trim();try{const e=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${s}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:n}]}],generationConfig:{temperature:.1}})});if(!e.ok){let o;try{o=await e.json()}catch{o={error:{message:`HTTP ${e.status}: ${e.statusText}`}}}const l=((a=o.error)==null?void 0:a.message)||`API Error: ${e.status}`;throw new Error(l)}let r=(await e.json()).candidates[0].content.parts[0].text;r=r.replace(/```json|```/g,"").trim();let c=JSON.parse(r);if(!Array.isArray(c)||c.length===0)throw new Error("Invalid format from API");return c.map((o,l)=>{const u=o.t||o.title||o.task||`Step ${l+1} for ${t.title}`,d=parseInt(o.min||o.minutes||o.time)||15,p=parseInt(o.p||o.priority)||3;return tt(t.id,u,d,p,l)})}catch(e){throw console.error("Gemini API Breakdown failed:",e),e}}function et(t){const s=Date.now(),n=new Date().getHours(),a=t.map(e=>{let i=0;if(i+=({new:5,oneoff:4,daily:3,periodic:2}[e.category]||2)*6,e.category==="periodic"&&e.intervalDays){const l=e.completedDates.length>0?new Date(e.completedDates[e.completedDates.length-1]).getTime():e.createdAt,d=(s-l)/(1e3*60*60*24)/e.intervalDays;i+=Math.min(d*12.5,25)}else if(e.category==="daily"){const l=new Date().toISOString().slice(0,10);e.completedDates.includes(l)||(i+=20)}else if(e.category==="new"){const l=(s-e.createdAt)/36e5;i+=Math.min(l*2,25)}n>=6&&n<=14&&(i+=10);const o=(s-e.createdAt)/(1e3*60*60*24);return i+=Math.max(10-o*2,0),{task:e,score:i}});return a.sort((e,i)=>i.score-e.score),a.map(e=>e.task)}function ht(t){return t.filter(n=>n.status!=="done").sort((n,a)=>n.order-a.order).slice(0,3)}function st(){return new Date().toISOString().slice(0,10)}async function gt(){const t=await E("lastActiveDate"),s=st();return t===s?!1:(await bt(s),await I("lastActiveDate",s),!0)}async function bt(t){const s=await D();for(const n of s)if(n.active)switch(n.category){case"daily":{const a=await S(n.id);for(const i of a)i.status==="done"?await A(i.id):await A(i.id);const e=C(n);for(const i of e)await L(i);break}case"periodic":{const a=n.completedDates.length>0?n.completedDates[n.completedDates.length-1]:null;let e=!1;if(!a)e=!0;else{const i=new Date(a);e=Math.floor((new Date(t)-i)/(1e3*60*60*24))>=(n.intervalDays||1)}if(e&&n.lastScheduledDate!==t){const i=await S(n.id);for(const c of i)await A(c.id);const r=C(n);for(const c of r)await L(c);n.lastScheduledDate=t,await O(n)}break}case"oneoff":{n.completedDates.includes(t)&&(n.active=!1,await O(n));break}case"new":{if((await S(n.id)).length===0){const e=C(n);for(const i of e)await L(i)}break}}}async function _(){const s=(await D()).filter(o=>o.active);if(s.length===0)return{subtasks:[],task:null};const n=et(s);let a=await E("currentFocusTaskIndex")||0;a>=n.length&&(a=0,await I("currentFocusTaskIndex",0));const e=n[a],i=await S(e.id);let r=await E(`focusBatch_${e.id}`),c=[];if(r&&Array.isArray(r)){const o=i.filter(u=>r.includes(u.id));!(o.length>0&&o.every(u=>u.status==="done"))&&o.length>0&&(c=o.sort((u,d)=>u.order-d.order))}if(c.length===0){const o=ht(i);if(o.length===0&&n.length>1)return await nt();o.length>0&&(c=o,await I(`focusBatch_${e.id}`,c.map(l=>l.id)))}return{subtasks:c,task:e}}async function nt(){const s=(await D()).filter(e=>e.active);if(s.length<=1)return await _();const n=et(s);let a=await E("currentFocusTaskIndex")||0;return a=(a+1)%n.length,await I("currentFocusTaskIndex",a),await _()}async function G(t){const{getSubtask:s,updateSubtask:n}=await q(async()=>{const{getSubtask:r,updateSubtask:c}=await Promise.resolve().then(()=>j);return{getSubtask:r,updateSubtask:c}},void 0),a=await s(t);if(!a)return;a.status="done",a.completedAt=Date.now(),await n(a);const i=(await S(a.taskId)).every(r=>r.status==="done");if(i){const{getTask:r,updateTask:c}=await q(async()=>{const{getTask:l,updateTask:u}=await Promise.resolve().then(()=>j);return{getTask:l,updateTask:u}},void 0),o=await r(a.taskId);o&&(o.completedDates.push(st()),o.category==="oneoff"&&(o.active=!1),await c(o))}return i}const at={};let B=null;function M(t,s){at[t]=s}function it(t){window.location.hash=t}function kt(t="#dashboard"){const s=()=>{const n=window.location.hash||t,a=at[n];a&&(B&&B.cleanup&&B.cleanup(),B=a())};window.addEventListener("hashchange",s),window.location.hash?s():window.location.hash=t}let P=!1;async function wt(){return"Notification"in window?Notification.permission==="granted"?(P=!0,!0):Notification.permission!=="denied"?(P=await Notification.requestPermission()==="granted",P):!1:!1}function Tt(t,s){P&&document.hidden&&new Notification(t,{body:s,icon:"/icons/icon-192.png",badge:"/icons/icon-192.png",vibrate:[200,100,200]}),y(t,s)}function y(t,s,n=4e3){const a=document.getElementById("toast-container");if(!a)return;const e=document.createElement("div");e.className="toast",e.innerHTML=`
    <div class="toast-title">${t}</div>
    ${s?`<div class="toast-body">${s}</div>`:""}
  `,a.appendChild(e),requestAnimationFrame(()=>e.classList.add("toast-show")),setTimeout(()=>{e.classList.remove("toast-show"),e.classList.add("toast-hide"),setTimeout(()=>e.remove(),400)},n)}let T=null,x=0,N=0;function St(t,s,n,a){x=s*60,N=s*60;const e=document.createElement("div");e.className="timer-overlay",e.id="timer-overlay",e.innerHTML=`
  ...
  `,Q(),H(),document.getElementById("btn-refresh").addEventListener("click",async()=>{const s=document.getElementById("btn-refresh");s.disabled=!0;const n=s.querySelector("svg");n.style.animation="spin 1s linear infinite",await nt(),await H(),s.disabled=!1,n.style.animation=""}),F=setInterval(Q,6e4),{cleanup:()=>{F&&clearInterval(F)}}}function Q(){const t=document.getElementById("greeting");if(!t)return;const s=new Date().getHours();let n="Good evening";s>=5&&s<12?n="Good morning":s>=12&&s<17&&(n="Good afternoon");const a=["🔥","💪","⚡","🚀","✨","🎯","💫"],e=a[Math.floor(Math.random()*a.length)];t.textContent=`${n} ${e}`}async function H(){const t=document.getElementById("focus-cards");if(!t)return;const{subtasks:s,task:n}=await _();if(!n||s.length===0){t.innerHTML=`
  ...
  `;c+=s.map((o,l)=>{const u=o.status==="done",d=o.priority>=4?"High":o.priority>=3?"Medium":"Low",p=o.priority>=4?"priority-high":o.priority>=3?"priority-med":"priority-low";return`
      <div class="focus-card card-enter ${u?"focus-card-done":""}" style="animation-delay: ${l*.1}s" data-subtask-id="${o.id}">
        <div class="focus-card-header">
          <span class="step-badge">Step ${o.order+1}</span>
          ${u?'<span class="done-badge">✅ Completed</span>':`<span class="priority-badge ${p}">${d}</span>`}
        </div>
        <div class="focus-card-title ${u?"title-strike":""}" style="font-size: 14px; margin-bottom: 10px;">${o.title}</div>
    ...
    `}).join(""),c+="</div>",t.innerHTML=c,t.querySelectorAll(".btn-start").forEach(o=>{o.addEventListener("click",l=>{const u=l.currentTarget.dataset.id,d=parseInt(l.currentTarget.dataset.minutes),p=s.find(f=>f.id===u);Lt(u,d,p?p.title:"Task")})}),t.querySelectorAll(".btn-done").forEach(o=>{o.addEventListener("click",async l=>{const u=l.currentTarget.dataset.id,d=l.currentTarget.closest(".focus-card");d.classList.add("subtask-done-animation");const p=d.querySelector(".btn-done");if(p&&(p.disabled=!0),await G(u)){setTimeout(()=>{Et(n.title)},100);return}const m=s.map(v=>v.id),{getSubtask:w}=await q(async()=>{const{getSubtask:v}=await Promise.resolve().then(()=>j);return{getSubtask:v}},void 0);if((await Promise.all(m.map(v=>w(v)))).every(v=>v&&v.status==="done"))setTimeout(()=>{It()},600);else{d.classList.add("focus-card-done");const v=d.querySelector(".focus-card-actions");v&&(v.style.display="none");const z=d.querySelector(".focus-card-header");if(z){const Y=z.querySelector(".priority-badge");Y&&(Y.outerHTML='<span class="done-badge">✅ Completed</span>')}const U=d.querySelector(".focus-card-title");U&&U.classList.add("title-strike"),y("✅ Step Complete!","Great work! Finish the set!")}})})}function Et(t){const s=document.createElement("div");s.className="full-task-completion-overlay",s.innerHTML=`
  ...
  `,document.body.appendChild(t),setTimeout(()=>{t.classList.add("show")},10),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>{t.remove(),H()},400)},2500)}function Lt(t,s,n){St(t,s,n,()=>{H()})}function $t(){const t=document.getElementById("app-content");t.innerHTML=`
  ...
  `;let s="daily";const n=document.getElementById("add-task-form"),a=document.getElementById("interval-group"),e=document.getElementById("interval-days");return document.querySelectorAll(".category-btn").forEach(i=>{i.addEventListener("click",()=>{document.querySelectorAll(".category-btn").forEach(r=>r.classList.remove("active")),i.classList.add("active"),s=i.dataset.cat,a.style.display=s==="periodic"?"block":"none"})}),document.getElementById("interval-dec").addEventListener("click",()=>{e.value=Math.max(2,parseInt(e.value)-1)}),document.getElementById("interval-inc").addEventListener("click",()=>{e.value=Math.min(365,parseInt(e.value)+1)}),n.addEventListener("submit",async i=>{i.preventDefault();const r=document.getElementById("task-title").value.trim();if(!r)return;const c=s==="periodic"?parseInt(e.value):null,o=ft(r,s,c);await R(o);const l=document.getElementById("btn-submit"),u=l.innerHTML;l.innerHTML='<svg class="spin" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Breaking it down...',l.disabled=!0;try{const d=await C(o);for(const m of d)await L(m);n.style.display="none";const p=document.getElementById("breakdown-preview");p.style.display="block";const f=document.getElementById("subtask-list");f.innerHTML=d.map((m,w)=>`
      ...
      `).join(""),y("✅ Task Added!",`"${r}" has been broken into ${d.length} subtasks`)}catch(d){console.error("Failed to break down:",d);try{await V(o.id)}catch(p){console.error("Failed to cleanup empty task:",p)}l.innerHTML=u,l.disabled=!1,y("❌ Error Breakdown",d.message||"Failed to break down task. Please check API Key or try again.")}document.getElementById("btn-go-dashboard").addEventListener("click",()=>{it("#dashboard")})}),{cleanup:()=>{}}}function Dt(){const t=document.getElementById("app-content");return t.innerHTML=`
  ...
  `,ot(),{cleanup:()=>{}}}async function ot(){const t=document.getElementById("task-list-content");if(!t)return;const s=await D(),n=s.filter(r=>r.active);if(n.length===0){t.innerHTML=`
      <div class="empty-state">
        <div class="empty-icon">📋</div>
        <h3>No tasks yet</h3>
        ...
        <div class="task-item card-enter" data-task-id="${o.id}">
          <div class="task-item-content">
            ...
            <div class="task-item-progress">
              ...
              <span class="progress-text">${u}/${d} subtasks</span>
            </div>
          </div>
    ...
    `),t.innerHTML=e,t.querySelectorAll(".btn-delete").forEach(r=>{r.addEventListener("click",async c=>{const o=c.currentTarget.dataset.taskId,l=c.currentTarget.closest(".task-item");confirm("Delete this task and all its subtasks?")&&(l.classList.add("card-exit"),setTimeout(async()=>{await V(o),y("🗑️ Deleted","Task removed"),ot()},300))})})}function Bt(){const t=document.getElementById("app-content");t.innerHTML=`
  ...
  `;const s=document.getElementById("api-key-form"),n=document.getElementById("api-key");E("geminiApiKey").then(i=>{i&&(n.value=i)}),s.addEventListener("submit",async i=>{i.preventDefault();const r=n.value.trim();if(!r){await I("geminiApiKey",null),y("⚠️ Cleared","API Key removed");return}await I("geminiApiKey",r),y("✅ Saved!","Your API Key is securely stored.")});const a=document.getElementById("btn-test-api"),e=document.getElementById("api-test-results");return a.addEventListener("click",async()=>{var r;const i=n.value.trim();if(!i)return y("⚠️ Error","Please enter an API Key first.");a.disabled=!0,a.innerHTML="Testing...",e.style.display="block",e.innerHTML='<p class="loading-text">Fetching authorized models...</p>';try{const c=await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${i}`);if(!c.ok){const d=await c.json();throw new Error(((r=d.error)==null?void 0:r.message)||`HTTP ${c.status}`)}const l=(await c.json()).models||[];if(l.length===0){e.innerHTML='<p class="error-text">No models found for this key.</p>';return}const u=l.filter(d=>d.name.includes("gemini"));e.innerHTML=`
        <p class="success-text">✅ Connection Successful!</p>
        <p class="results-hint">Found these Gemini models on your key:</p>
        <ul class="model-list">
          ${u.map(d=>`<li><code>${d.name.replace("models/","")}</code></li>`).join("")}
        </ul>
        <p class="results-hint">If "gemini-1.5-flash" isn't listed, please enable it in Google AI Studio.</p>
      `,y("✅ Connected",`Found ${u.length} Gemini models!`)}catch(c){console.error(c),e.innerHTML=`<p class="error-text">❌ Connection Failed: ${c.message}</p>`,y("❌ Failed","API Key check failed")}finally{a.disabled=!1,a.innerHTML="Test Connection"}}),{cleanup:()=>{}}}async function Mt(){const t=document.getElementById("app");t.innerHTML=`
    <div class="app-shell">
  ...
  `,await g(),await gt(),wt(),M("#dashboard",xt),M("#add",$t),M("#tasks",Dt),M("#settings",Bt);const s=document.querySelectorAll(".nav-btn");function n(){const a=window.location.hash||"#dashboard";s.forEach(e=>{e.classList.toggle("active",e.dataset.route===a)})}s.forEach(a=>{a.addEventListener("click",()=>{window.location.hash=a.dataset.route})}),window.addEventListener("hashchange",n),kt("#dashboard"),n()}Mt().catch(console.error);
