...
[
  ...
  `.trim();try{const e=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${s}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:n}]}],generationConfig:{temperature:.1}})});if(!e.ok){let o;try{o=await e.json()}catch{o={error:{message:`HTTP ${e.status}: ${e.statusText}`}}}const l=((a=o.error)==null?void 0:a.message)||`API Error: ${e.status}`;throw new Error(l)}let r=(await e.json()).candidates[0].content.parts[0].text;r=r.replace(/```json|```/g,"").trim();let c=JSON.parse(r);if(!Array.isArray(c)||c.length===0)throw new Error("Invalid format from API");return c.map((o,l)=>{const d=o.t||o.title||o.task||`Step ${l+1} for ${t.title}`,u=parseInt(o.min||o.minutes||o.time)||15,p=parseInt(o.p||o.priority)||3;return tt(t.id,d,u,p,l)})}catch(e){throw console.error("Gemini API Breakdown failed:",e),e}}function et(t){const s=Date.now(),n=new Date().getHours(),a=t.map(e=>{let i=0;if(i+=({new:5,oneoff:4,daily:3,periodic:2}[e.category]||2)*6,e.category==="periodic"&&e.intervalDays){const l=e.completedDates.length>0?new Date(e.completedDates[e.completedDates.length-1]).getTime():e.createdAt,u=(s-l)/(1e3*60*60*24)/e.intervalDays;i+=Math.min(u*12.5,25)}else if(e.category==="daily"){const l=new Date().toISOString().slice(0,10);e.completedDates.includes(l)||(i+=20)}else if(e.category==="new"){const l=(s-e.createdAt)/36e5;i+=Math.min(l*2,25)}n>=6&&n<=14&&(i+=10);const o=(s-e.createdAt)/(1e3*60*60*24);return i+=Math.max(10-o*2,0),{task:e,score:i}});return a.sort((e,i)=>i.score-e.score),a.map(e=>e.task)}function yt(t){return t.filter(n=>n.status!=="done").sort((n,a)=>n.order-a.order).slice(0,3)}function st(){return new Date().toISOString().slice(0,10)}async function gt(){const t=await E("lastActiveDate"),s=st();return t===s?!1:(await bt(s),await I("lastActiveDate",s),!0)}async function bt(t){const s=await B();for(const n of s)if(n.active)switch(n.category){case"daily":{const a=await S(n.id);for(const i of a)i.status==="done"?await A(i.id):await A(i.id);const e=C(n);for(const i of e)await $(i);break}case"periodic":{const a=n.completedDates.length>0?n.completedDates[n.completedDates.length-1]:null;let e=!1;if(!a)e=!0;else{const i=new Date(a);e=Math.floor((new Date(t)-i)/(1e3*60*60*24))>=(n.intervalDays||1)}if(e&&n.lastScheduledDate!==t){const i=await S(n.id);for(const c of i)await A(c.id);const r=C(n);for(const c of r)await $(c);n.lastScheduledDate=t,await O(n)}break}case"oneoff":{n.completedDates.includes(t)&&(n.active=!1,await O(n));break}case"new":{if((await S(n.id)).length===0){const e=C(n);for(const i of e)await $(i)}break}}}async function q(){const s=(await B()).filter(o=>o.active);if(s.length===0)return{subtasks:[],task:null};const n=et(s);let a=await E("currentFocusTaskIndex")||0;a>=n.length&&(a=0,await I("currentFocusTaskIndex",0));const e=n[a],i=await S(e.id);let r=await E(`focusBatch_${e.id}`),c=[];if(r&&Array.isArray(r)){const o=i.filter(d=>r.includes(d.id));!(o.length>0&&o.every(d=>d.status==="done"))&&o.length>0&&(c=o.sort((d,u)=>d.order-u.order))}if(c.length===0){const o=yt(i);if(o.length===0&&n.length>1)return await nt();o.length>0&&(c=o,await I(`focusBatch_${e.id}`,c.map(l=>l.id)))}return{subtasks:c,task:e}}async function nt(){const s=(await B()).filter(e=>e.active);if(s.length<=1)return await q();const n=et(s);let a=await E("currentFocusTaskIndex")||0;return a=(a+1)%n.length,await I("currentFocusTaskIndex",a),await q()}async function R(t){const{getSubtask:s,updateSubtask:n}=await N(async()=>{const{getSubtask:r,updateSubtask:c}=await Promise.resolve().then(()=>H);return{getSubtask:r,updateSubtask:c}},void 0),a=await s(t);if(!a)return;a.status="done",a.completedAt=Date.now(),await n(a);const i=(await S(a.taskId)).every(r=>r.status==="done");if(i){const{getTask:r,updateTask:c}=await N(async()=>{const{getTask:l,updateTask:d}=await Promise.resolve().then(()=>H);return{getTask:l,updateTask:d}},void 0),o=await r(a.taskId);o&&(o.completedDates.push(st()),o.category==="oneoff"&&(o.active=!1),await c(o))}return i}const at={};let D=null;function M(t,s){at[t]=s}function it(t){window.location.hash=t}function kt(t="#dashboard"){const s=()=>{const n=window.location.hash||t,a=at[n];a&&(D&&D.cleanup&&D.cleanup(),D=a())};window.addEventListener("hashchange",s),window.location.hash?s():window.location.hash=t}let P=!1;async function wt(){return"Notification"in window?Notification.permission==="granted"?(P=!0,!0):Notification.permission!=="denied"?(P=await Notification.requestPermission()==="granted",P):!1:!1}function Tt(t,s){P&&document.hidden&&new Notification(t,{body:s,icon:"/icons/icon-192.png",badge:"/icons/icon-192.png",vibrate:[200,100,200]}),h(t,s)}function h(t,s,n=4e3){const a=document.getElementById("toast-container");if(!a)return;const e=document.createElement("div");e.className="toast",e.innerHTML=`
    <div class="toast-title">${t}</div>
    ${s?`<div class="toast-body">${s}</div>`:""}
  `,a.appendChild(e),requestAnimationFrame(()=>e.classList.add("toast-show")),setTimeout(()=>{e.classList.remove("toast-show"),e.classList.add("toast-hide"),setTimeout(()=>e.remove(),400)},n)}let T=null,x=0,_=0;function St(t,s,n,a){x=s*60,_=s*60;const e=document.createElement("div");e.className="timer-overlay",e.id="timer-overlay",e.innerHTML=`
  ...
  `,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("timer-overlay-show"));let i=!1;const r=document.getElementById("timer-display"),c=document.getElementById("timer-progress-ring"),o=2*Math.PI*88,l=document.getElementById("btn-pause");function d(){r&&(r.textContent=X(x));const y=1-x/_,v=o*y;c&&(c.style.strokeDashoffset=v)}function u(){i||(x--,d(),x<=0&&(clearInterval(T),T=null,p()))}function p(){Tt("⏰ Time's Up!",`Timer for "${n}" has ended.`),"vibrate"in navigator&&navigator.vibrate([200,100,200,100,200]),document.querySelector(".timer-controls").style.display="none",document.getElementById("timeup-panel").style.display="block"}function f(y){x+=y*60,_+=y*60,document.getElementById("timeup-panel").style.display="none",document.querySelector(".timer-controls").style.display="flex",d(),T||(T=setInterval(u,1e3)),h("⏱️ Time Added",`+${y} minutes added`)}function m(y){T&&(clearInterval(T),T=null),e.classList.remove("timer-overlay-show"),e.classList.add("timer-overlay-hide"),setTimeout(()=>{e.remove(),a&&a()},400)}T=setInterval(u,1e3),l.addEventListener("click",()=>{i=!i,l.innerHTML=i?'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polygon points="5 3 19 12 5 21 5 3"/></svg> Resume':'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> Pause',l.classList.toggle("btn-resume",i)}),document.getElementById("btn-finish-early").addEventListener("click",async()=>{await R(t),h("✅ Done!","Great focus session!"),m()}),document.getElementById("btn-done-final").addEventListener("click",async()=>{await R(t),h("🎉 Complete!","You crushed it!"),m()}),e.querySelectorAll(".btn-add-time[data-minutes]").forEach(y=>{y.addEventListener("click",()=>{f(parseInt(y.dataset.minutes))})});const w=document.getElementById("random-slider"),K=document.getElementById("random-value");w.addEventListener("input",()=>{K.textContent=`${w.value} min`}),document.getElementById("btn-random-add").addEventListener("click",()=>{const y=parseInt(w.value),v=Math.max(1,Math.floor(Math.random()*y)+1);f(v),h("🎲 Random Time",`Added ${v} random minutes!`)})}function X(t){const s=Math.floor(t/60),n=t%60;return`${s.toString().padStart(2,"0")}:${n.toString().padStart(2,"0")}`}let j=null;function xt(){const t=document.getElementById("app-content");return t.innerHTML=`
    <div class="dashboard">
  ...
  `,Q(),F(),document.getElementById("btn-refresh").addEventListener("click",async()=>{const s=document.getElementById("btn-refresh");s.disabled=!0;const n=s.querySelector("svg");n.style.animation="spin 1s linear infinite",await nt(),await F(),s.disabled=!1,n.style.animation=""}),j=setInterval(Q,6e4),{cleanup:()=>{j&&clearInterval(j)}}}function Q(){const t=document.getElementById("greeting");if(!t)return;const s=new Date().getHours();let n="Good evening";s>=5&&s<12?n="Good morning":s>=12&&s<17&&(n="Good afternoon");const a=["🔥","💪","⚡","🚀","✨","🎯","💫"],e=a[Math.floor(Math.random()*a.length)];t.textContent=`${n} ${e}`}async function F(){const t=document.getElementById("focus-cards");if(!t)return;const{subtasks:s,task:n}=await q();if(!n||s.length===0){t.innerHTML=`
  ...
  `,document.body.appendChild(t),setTimeout(()=>{t.classList.add("show")},10),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>{t.remove(),F()},400)},2500)}function $t(t,s,n){St(t,s,n,()=>{F()})}function Lt(){const t=document.getElementById("app-content");t.innerHTML=`
  ...
  `,ot(),{cleanup:()=>{}}}async function ot(){const t=document.getElementById("task-list-content");if(!t)return;const s=await B(),n=s.filter(r=>r.active);if(n.length===0){t.innerHTML=`
      <div class="empty-state">
        <div class="empty-icon">📋</div>
        <h3>No tasks yet</h3>
  ...
  `;const s=document.getElementById("api-key-form"),n=document.getElementById("api-key");E("geminiApiKey").then(i=>{i&&(n.value=i)}),s.addEventListener("submit",async i=>{i.preventDefault();const r=n.value.trim();if(!r){await I("geminiApiKey",null),h("⚠️ Cleared","API Key removed");return}await I("geminiApiKey",r),h("✅ Saved!","Your API Key is securely stored.")});const a=document.getElementById("btn-test-api"),e=document.getElementById("api-test-results");return a.addEventListener("click",async()=>{var r;const i=n.value.trim();if(!i)return h("⚠️ Error","Please enter an API Key first.");a.disabled=!0,a.innerHTML="Testing...",e.style.display="block",e.innerHTML='<p class="loading-text">Fetching authorized models...</p>';try{const c=await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${i}`);if(!c.ok){const u=await c.json();throw new Error(((r=u.error)==null?void 0:r.message)||`HTTP ${c.status}`)}const l=(await c.json()).models||[];if(l.length===0){e.innerHTML='<p class="error-text">No models found for this key.</p>';return}const d=l.filter(u=>u.name.includes("gemini"));e.innerHTML=`
      ...
      `,h("✅ Connected",`Found ${d.length} Gemini models!`)}catch(c){console.error(c),e.innerHTML=`<p class="error-text">❌ Connection Failed: ${c.message}</p>`,h("❌ Failed","API Key check failed")}finally{a.disabled=!1,a.innerHTML="Test Connection"}}),{cleanup:()=>{}}}async function At(){const t=document.getElementById("app");t.innerHTML=`
    <div class="app-shell">
  ...
  `,await g(),await gt(),wt(),M("#dashboard",xt),M("#add",Lt),M("#tasks",Bt),M("#settings",Mt);const s=document.querySelectorAll(".nav-btn");function n(){const a=window.location.hash||"#dashboard";s.forEach(e=>{e.classList.toggle("active",e.dataset.route===a)})}s.forEach(a=>{a.addEventListener("click",()=>{window.location.hash=a.dataset.route})}),window.addEventListener("hashchange",n),kt("#dashboard"),n()}At().catch(console.error);
