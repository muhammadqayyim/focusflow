(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))a(e);new MutationObserver(e=>{for(const i of e)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function n(e){const i={};return e.integrity&&(i.integrity=e.integrity),e.referrerPolicy&&(i.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?i.credentials="include":e.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(e){if(e.ep)return;e.ep=!0;const i=n(e);fetch(e.href,i)}})();const lt="focusflow",dt=1;let B=null;function g(){return new Promise((t,s)=>{if(B)return t(B);const n=indexedDB.open(lt,dt);n.onupgradeneeded=a=>{const e=a.target.result;if(e.objectStoreNames.contains("tasks")||e.createObjectStore("tasks",{keyPath:"id"}).createIndex("category","category",{unique:!1}),!e.objectStoreNames.contains("subtasks")){const i=e.createObjectStore("subtasks",{keyPath:"id"});i.createIndex("taskId","taskId",{unique:!1}),i.createIndex("status","status",{unique:!1})}e.objectStoreNames.contains("appState")||e.createObjectStore("appState",{keyPath:"key"})},n.onsuccess=a=>{B=a.target.result,t(B)},n.onerror=a=>s(a.target.error)})}function b(t,s="readonly"){return B.transaction(t,s).objectStore(t)}function k(t){return new Promise((s,n)=>{t.onsuccess=()=>s(t.result),t.onerror=()=>n(t.error)})}async function G(t){return await g(),k(b("tasks","readwrite").put(t))}async function ut(t){return await g(),k(b("tasks").get(t))}async function D(){return await g(),k(b("tasks").getAll())}async function V(t){await g();const s=await S(t),n=b("subtasks","readwrite");for(const a of s)n.delete(a.id);return k(b("tasks","readwrite").delete(t))}async function R(t){return G(t)}async function $(t){return await g(),k(b("subtasks","readwrite").put(t))}async function pt(t){return await g(),k(b("subtasks").get(t))}async function S(t){await g();const s=b("subtasks").index("taskId");return k(s.getAll(t))}async function mt(t){return $(t)}async function P(t){return await g(),k(b("subtasks","readwrite").delete(t))}async function I(t){await g();const s=await k(b("appState").get(t));return s?s.value:null}async function x(t,s){return await g(),k(b("appState","readwrite").put({key:t,value:s}))}const F=Object.freeze(Object.defineProperty({__proto__:null,addSubtask:$,addTask:G,deleteSubtask:P,deleteTask:V,getAllTasks:D,getState:I,getSubtask:pt,getSubtasksByTask:S,getTask:ut,openDB:g,setState:x,updateSubtask:mt,updateTask:R},Symbol.toStringTag,{value:"Module"})),vt="modulepreload",ft=function(t){return"/focusflow/"+t},Z={},L=function(s,n,a){let e=Promise.resolve();if(n&&n.length>0){let r=function(l){return Promise.all(l.map(d=>Promise.resolve(d).then(u=>({status:"fulfilled",value:u}),u=>({status:"rejected",reason:u}))))};document.getElementsByTagName("link");const c=document.querySelector("meta[property=csp-nonce]"),o=(c==null?void 0:c.nonce)||(c==null?void 0:c.getAttribute("nonce"));e=r(n.map(l=>{if(l=ft(l),l in Z)return;Z[l]=!0;const d=l.endsWith(".css"),u=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${u}`))return;const p=document.createElement("link");if(p.rel=d?"stylesheet":vt,d||(p.as="script"),p.crossOrigin="",p.href=l,o&&p.setAttribute("nonce",o),document.head.appendChild(p),d)return new Promise((f,m)=>{p.addEventListener("load",f),p.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${l}`)))})}))}function i(r){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=r,window.dispatchEvent(c),!c.defaultPrevented)throw r}return e.then(r=>{for(const c of r||[])c.status==="rejected"&&i(c.reason);return s().catch(i)})};let yt=0;function nt(){return Date.now().toString(36)+"-"+(++yt).toString(36)+"-"+Math.random().toString(36).slice(2,7)}function ht(t,s,n=null){return{id:nt(),title:t.trim(),category:s,intervalDays:n,createdAt:Date.now(),completedDates:[],lastScheduledDate:null,active:!0}}function at(t,s,n,a,e=0){return{id:nt(),taskId:t,title:s.trim(),estimatedMinutes:n,priority:a,order:e,status:"pending",completedAt:null,addedTimeMinutes:0}}const tt={daily:t=>[{t:`Set up & prepare to start: ${t}`,min:5,p:3},{t:`Focus on the main work: ${t}`,min:20,p:5},{t:`Wrap up & quick review of: ${t}`,min:5,p:4}],periodic:t=>[{t:`Recall where you left off & review progress on: ${t}`,min:5,p:3},{t:`Do the core work session: ${t}`,min:20,p:5},{t:`Document progress & set reminders for next time: ${t}`,min:5,p:4}],oneoff:t=>[{t:`Research & figure out the approach for: ${t}`,min:10,p:3},{t:`Execute the main action: ${t}`,min:20,p:5},{t:`Verify completion & do any follow-ups for: ${t}`,min:10,p:4}],new:t=>[{t:`Understand requirements & ask clarifying questions about: ${t}`,min:10,p:3},{t:`Create first draft / initial attempt: ${t}`,min:20,p:5},{t:`Review your work & refine: ${t}`,min:10,p:4}]};function gt(t){return console.warn("Using fallback breakdown for:",t.title),(tt[t.category]||tt.daily)(t.title).map((a,e)=>at(t.id,a.t,a.min,a.p,e))}async function C(t){var a;const s=await I("geminiApiKey");if(!s)return gt(t);const n=`
...
[
  ...
  `.trim();try{const e=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${s}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:n}]}],generationConfig:{temperature:.1}})});if(!e.ok){let o;try{o=await e.json()}catch{o={error:{message:`HTTP ${e.status}: ${e.statusText}`}}}const l=((a=o.error)==null?void 0:a.message)||`API Error: ${e.status}`;throw new Error(l)}let r=(await e.json()).candidates[0].content.parts[0].text;r=r.replace(/```json|```/g,"").trim();let c=JSON.parse(r);if(!Array.isArray(c)||c.length===0)throw new Error("Invalid format from API");return c.map((o,l)=>{const d=o.t||o.title||o.task||`Step ${l+1} for ${t.title}`,u=parseInt(o.min||o.minutes||o.time)||15,p=parseInt(o.p||o.priority)||3;return at(t.id,d,u,p,l)})}catch(e){throw console.error("Gemini API Breakdown failed:",e),e}}function K(t){const s=Date.now(),n=new Date().getHours(),a=t.map(e=>{let i=0;if(i+=({new:5,oneoff:4,daily:3,periodic:2}[e.category]||2)*6,e.category==="periodic"&&e.intervalDays){const l=e.completedDates.length>0?new Date(e.completedDates[e.completedDates.length-1]).getTime():e.createdAt,u=(s-l)/(1e3*60*60*24)/e.intervalDays;i+=Math.min(u*12.5,25)}else if(e.category==="daily"){const l=new Date().toISOString().slice(0,10);e.completedDates.includes(l)||(i+=20)}else if(e.category==="new"){const l=(s-e.createdAt)/36e5;i+=Math.min(l*2,25)}n>=6&&n<=14&&(i+=10);const o=(s-e.createdAt)/(1e3*60*60*24);return i+=Math.max(10-o*2,0),{task:e,score:i}});return a.sort((e,i)=>i.score-e.score),a.map(e=>e.task)}function bt(t){return t.filter(n=>n.status!=="done").sort((n,a)=>n.order-a.order).slice(0,3)}function z(){return new Date().toISOString().slice(0,10)}async function it(){const t=await I("lastActiveDate"),s=z();return t===s?!1:(await kt(s),await x("lastActiveDate",s),!0)}async function kt(t){const s=await D();for(const n of s)if(n.active)switch(n.category){case"daily":{const a=await S(n.id);for(const i of a)i.status==="done"?await P(i.id):await P(i.id);const e=C(n);for(const i of e)await $(i);break}case"periodic":{const a=n.completedDates.length>0?n.completedDates[n.completedDates.length-1]:null;let e=!1;if(!a)e=!0;else{const i=new Date(a);e=Math.floor((new Date(t)-i)/(1e3*60*60*24))>=(n.intervalDays||1)}if(e&&n.lastScheduledDate!==t){const i=await S(n.id);for(const c of i)await P(c.id);const r=C(n);for(const c of r)await $(c);n.lastScheduledDate=t,await R(n)}break}case"oneoff":{n.completedDates.includes(t)&&(n.active=!1,await R(n));break}case"new":{if((await S(n.id)).length===0){const e=C(n);for(const i of e)await $(i)}break}}}async function H(){const s=(await D()).filter(o=>o.active);if(s.length===0)return{subtasks:[],task:null};const n=K(s);let a=await I("currentFocusTaskIndex")||0;a>=n.length&&(a=0,await x("currentFocusTaskIndex",0));const e=n[a],i=await S(e.id);let r=await I(`focusBatch_${e.id}`),c=[];if(r&&Array.isArray(r)){const o=i.filter(d=>r.includes(d.id));!(o.length>0&&o.every(d=>d.status==="done"))&&o.length>0&&(c=o.sort((d,u)=>d.order-u.order))}if(c.length===0){const o=bt(i);if(o.length===0&&n.length>1)return await U();o.length>0&&(c=o,await x(`focusBatch_${e.id}`,c.map(l=>l.id)))}return{subtasks:c,task:e}}async function U(){const s=(await D()).filter(e=>e.active);if(s.length<=1)return await H();const n=K(s);let a=await I("currentFocusTaskIndex")||0;return a=(a+1)%n.length,await x("currentFocusTaskIndex",a),await H()}async function O(t){const{getSubtask:s,updateSubtask:n}=await L(async()=>{const{getSubtask:r,updateSubtask:c}=await Promise.resolve().then(()=>F);return{getSubtask:r,updateSubtask:c}},void 0),a=await s(t);if(!a)return;a.status="done",a.completedAt=Date.now(),await n(a);const i=(await S(a.taskId)).every(r=>r.status==="done");if(i){const{getTask:r,updateTask:c}=await L(async()=>{const{getTask:l,updateTask:d}=await Promise.resolve().then(()=>F);return{getTask:l,updateTask:d}},void 0),o=await r(a.taskId);o&&(o.completedDates.push(z()),o.category==="oneoff"&&(o.active=!1),await c(o))}return i}async function wt(t){const n=(await D()).filter(i=>i.active),e=K(n).findIndex(i=>i.id===t);e!==-1&&await x("currentFocusTaskIndex",e)}const Tt=Object.freeze(Object.defineProperty({__proto__:null,checkNewDay:it,completeSubtask:O,getFocusSubtasks:H,setFocusTask:wt,shuffleFocusTask:U,todayStr:z},Symbol.toStringTag,{value:"Module"})),ot={};let A=null;function M(t,s){ot[t]=s}function Y(t){window.location.hash=t}function rt(t="#dashboard"){const s=()=>{const n=window.location.hash||t,a=ot[n];a&&(A&&A.cleanup&&A.cleanup(),A=a())};window.addEventListener("hashchange",s),window.location.hash?s():window.location.hash=t}const St=Object.freeze(Object.defineProperty({__proto__:null,initRouter:rt,navigate:Y,registerRoute:M},Symbol.toStringTag,{value:"Module"}));let _=!1;async function xt(){return"Notification"in window?Notification.permission==="granted"?(_=!0,!0):Notification.permission!=="denied"?(_=await Notification.requestPermission()==="granted",_):!1:!1}function Et(t,s){_&&document.hidden&&new Notification(t,{body:s,icon:"/icons/icon-192.png",badge:"/icons/icon-192.png",vibrate:[200,100,200]}),y(t,s)}function y(t,s,n=4e3){const a=document.getElementById("toast-container");if(!a)return;const e=document.createElement("div");e.className="toast",e.innerHTML=`
    <div class="toast-title">${t}</div>
    ${s?`<div class="toast-body">${s}</div>`:""}
  `,a.appendChild(e),requestAnimationFrame(()=>e.classList.add("toast-show")),setTimeout(()=>{e.classList.remove("toast-show"),e.classList.add("toast-hide"),setTimeout(()=>e.remove(),400)},n)}let T=null,E=0,j=0;function It(t,s,n,a){E=s*60,j=s*60;const e=document.createElement("div");e.className="timer-overlay",e.id="timer-overlay",e.innerHTML=`
  ...
  `,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("timer-overlay-show"));let i=!1;const r=document.getElementById("timer-display"),c=document.getElementById("timer-progress-ring"),o=2*Math.PI*88,l=document.getElementById("btn-pause");function d(){r&&(r.textContent=et(E));const h=1-E/j,v=o*h;c&&(c.style.strokeDashoffset=v)}function u(){i||(E--,d(),E<=0&&(clearInterval(T),T=null,p()))}function p(){Et("⏰ Time's Up!",`Timer for "${n}" has ended.`),"vibrate"in navigator&&navigator.vibrate([200,100,200,100,200]),document.querySelector(".timer-controls").style.display="none",document.getElementById("timeup-panel").style.display="block"}function f(h){E+=h*60,j+=h*60,document.getElementById("timeup-panel").style.display="none",document.querySelector(".timer-controls").style.display="flex",d(),T||(T=setInterval(u,1e3)),y("⏱️ Time Added",`+${h} minutes added`)}function m(h){T&&(clearInterval(T),T=null),e.classList.remove("timer-overlay-show"),e.classList.add("timer-overlay-hide"),setTimeout(()=>{e.remove(),a&&a()},400)}T=setInterval(u,1e3),l.addEventListener("click",()=>{i=!i,l.innerHTML=i?'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polygon points="5 3 19 12 5 21 5 3"/></svg> Resume':'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> Pause',l.classList.toggle("btn-resume",i)}),document.getElementById("btn-finish-early").addEventListener("click",async()=>{await O(t),y("✅ Done!","Great focus session!"),m()}),document.getElementById("btn-done-final").addEventListener("click",async()=>{await O(t),y("🎉 Complete!","You crushed it!"),m()}),e.querySelectorAll(".btn-add-time[data-minutes]").forEach(h=>{h.addEventListener("click",()=>{f(parseInt(h.dataset.minutes))})});const w=document.getElementById("random-slider"),W=document.getElementById("random-value");w.addEventListener("input",()=>{W.textContent=`${w.value} min`}),document.getElementById("btn-random-add").addEventListener("click",()=>{const h=parseInt(w.value),v=Math.max(1,Math.floor(Math.random()*h)+1);f(v),y("🎲 Random Time",`Added ${v} random minutes!`)})}function et(t){const s=Math.floor(t/60),n=t%60;return`${s.toString().padStart(2,"0")}:${n.toString().padStart(2,"0")}`}let q=null;function $t(){const t=document.getElementById("app-content");return t.innerHTML=`
    <div class="dashboard">
  ...
  `,st(),N(),document.getElementById("btn-refresh").addEventListener("click",async()=>{const s=document.getElementById("btn-refresh");s.disabled=!0;const n=s.querySelector("svg");n.style.animation="spin 1s linear infinite",await U(),await N(),s.disabled=!1,n.style.animation=""}),q=setInterval(st,6e4),{cleanup:()=>{q&&clearInterval(q)}}}function st(){const t=document.getElementById("greeting");if(!t)return;const s=new Date().getHours();let n="Good evening";s>=5&&s<12?n="Good morning":s>=12&&s<17&&(n="Good afternoon");const a=["🔥","💪","⚡","🚀","✨","🎯","💫"],e=a[Math.floor(Math.random()*a.length)];t.textContent=`${n} ${e}`}async function N(){const t=document.getElementById("focus-cards");if(!t)return;const{subtasks:s,task:n}=await H();if(!n||s.length===0){t.innerHTML=`
  ...
  `;c+=s.map((o,l)=>{const d=o.status==="done",u=o.priority>=4?"High":o.priority>=3?"Medium":"Low",p=o.priority>=4?"priority-high":o.priority>=3?"priority-med":"priority-low";return`
    ...
    `}).join(""),c+="</div>",t.innerHTML=c,t.querySelectorAll(".btn-start").forEach(o=>{o.addEventListener("click",l=>{const d=l.currentTarget.dataset.id,u=parseInt(l.currentTarget.dataset.minutes),p=s.find(f=>f.id===d);Bt(d,u,p?p.title:"Task")})}),t.querySelectorAll(".btn-done").forEach(o=>{o.addEventListener("click",async l=>{const d=l.currentTarget.dataset.id,u=l.currentTarget.closest(".focus-card");u.classList.add("subtask-done-animation");const p=u.querySelector(".btn-done");if(p&&(p.disabled=!0),await O(d)){setTimeout(()=>{Lt(n.title)},100);return}const m=s.map(v=>v.id),{getSubtask:w}=await L(async()=>{const{getSubtask:v}=await Promise.resolve().then(()=>F);return{getSubtask:v}},void 0);if((await Promise.all(m.map(v=>w(v)))).every(v=>v&&v.status==="done"))setTimeout(()=>{Dt()},600);else{u.classList.add("focus-card-done");const v=u.querySelector(".focus-card-actions");v&&(v.style.display="none");const J=u.querySelector(".focus-card-header");if(J){const Q=J.querySelector(".priority-badge");Q&&(Q.outerHTML='<span class="done-badge">✅ Completed</span>')}const X=u.querySelector(".focus-card-title");X&&X.classList.add("title-strike"),y("✅ Step Complete!","Great work! Finish the set!")}})})}function Lt(t){const s=document.createElement("div");s.className="full-task-completion-overlay",s.innerHTML=`
  ...
  `,ct(),{cleanup:()=>{}}}async function ct(){const t=document.getElementById("task-list-content");if(!t)return;const s=await D(),n=s.filter(r=>r.active);if(n.length===0){t.innerHTML=`
      <div class="empty-state">
        <div class="empty-icon">📋</div>
        <h3>No tasks yet</h3>
        ...
        <div class="task-item card-enter" data-task-id="${o.id}">
          <div class="task-item-click-area">
            <div class="task-item-content">
              <div class="task-item-title" id="title-${o.id}">
              ...
              <div class="task-item-progress">
                <div class="progress-bar">
                  <div class="progress-fill" id="progress-${o.id}" style="width: ${p}%"></div>
                </div>
                <span class="progress-text" id="text-${o.id}">${d}/${u} subtasks</span>
              </div>
            </div>
          ...
          <div class="subtasks-expand-container" id="subs-container-${o.id}"></div>
    ...
    `),t.innerHTML=e,t.querySelectorAll(".task-item-click-area").forEach(r=>{r.addEventListener("click",c=>{const o=c.currentTarget.closest(".task-item"),l=o.dataset.taskId,d=document.getElementById(`subs-container-${l}`);o.classList.toggle("expanded")&&d.innerHTML===""&&Pt(l,d)})}),t.querySelectorAll(".btn-delete").forEach(r=>{r.addEventListener("click",async c=>{const o=c.currentTarget.dataset.taskId,l=c.currentTarget.closest(".task-item");confirm("Delete this task and all its subtasks?")&&(l.classList.add("card-exit"),setTimeout(async()=>{await V(o),y("🗑️ Deleted","Task removed"),ct()},300))})})}async function Pt(t,s){const{getSubtasksByTask:n,updateSubtask:a}=await L(async()=>{const{getSubtasksByTask:i,updateSubtask:r}=await Promise.resolve().then(()=>F);return{getSubtasksByTask:i,updateSubtask:r}},void 0),e=await n(t);if(e.length===0){s.innerHTML='<div class="no-subs">No subtasks found</div>';return}s.innerHTML=`
  ...
  `,s.querySelector(".btn-focus").addEventListener("click",async i=>{i.stopPropagation();const{setFocusTask:r}=await L(async()=>{const{setFocusTask:o}=await Promise.resolve().then(()=>Tt);return{setFocusTask:o}},void 0),{navigate:c}=await L(async()=>{const{navigate:o}=await Promise.resolve().then(()=>St);return{navigate:o}},void 0);await r(t),c("#dashboard")})}function Ct(){const t=document.getElementById("app-content");t.innerHTML=`
  ...
  `;const s=document.getElementById("api-key-form"),n=document.getElementById("api-key");I("geminiApiKey").then(i=>{i&&(n.value=i)}),s.addEventListener("submit",async i=>{i.preventDefault();const r=n.value.trim();if(!r){await x("geminiApiKey",null),y("⚠️ Cleared","API Key removed");return}await x("geminiApiKey",r),y("✅ Saved!","Your API Key is securely stored.")});const a=document.getElementById("btn-test-api"),e=document.getElementById("api-test-results");return a.addEventListener("click",async()=>{var r;const i=n.value.trim();if(!i)return y("⚠️ Error","Please enter an API Key first.");a.disabled=!0,a.innerHTML="Testing...",e.style.display="block",e.innerHTML='<p class="loading-text">Fetching authorized models...</p>';try{const c=await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${i}`);if(!c.ok){const u=await c.json();throw new Error(((r=u.error)==null?void 0:r.message)||`HTTP ${c.status}`)}const l=(await c.json()).models||[];if(l.length===0){e.innerHTML='<p class="error-text">No models found for this key.</p>';return}const d=l.filter(u=>u.name.includes("gemini"));e.innerHTML=`
      ...
      `,y("✅ Connected",`Found ${d.length} Gemini models!`)}catch(c){console.error(c),e.innerHTML=`<p class="error-text">❌ Connection Failed: ${c.message}</p>`,y("❌ Failed","API Key check failed")}finally{a.disabled=!1,a.innerHTML="Test Connection"}}),{cleanup:()=>{}}}async function _t(){const t=document.getElementById("app");t.innerHTML=`
    <div class="app-shell">
  ...
  `,await g(),await it(),xt(),M("#dashboard",$t),M("#add",Mt),M("#tasks",At),M("#settings",Ct);const s=document.querySelectorAll(".nav-btn");function n(){const a=window.location.hash||"#dashboard";s.forEach(e=>{e.classList.toggle("active",e.dataset.route===a)})}s.forEach(a=>{a.addEventListener("click",()=>{window.location.hash=a.dataset.route})}),window.addEventListener("hashchange",n),rt("#dashboard"),n()}_t().catch(console.error);
