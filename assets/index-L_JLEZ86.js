(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))a(e);new MutationObserver(e=>{for(const i of e)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function s(e){const i={};return e.integrity&&(i.integrity=e.integrity),e.referrerPolicy&&(i.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?i.credentials="include":e.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(e){if(e.ep)return;e.ep=!0;const i=s(e);fetch(e.href,i)}})();const it="focusflow",ot=1;let $=null;function g(){return new Promise((t,n)=>{if($)return t($);const s=indexedDB.open(it,ot);s.onupgradeneeded=a=>{const e=a.target.result;if(e.objectStoreNames.contains("tasks")||e.createObjectStore("tasks",{keyPath:"id"}).createIndex("category","category",{unique:!1}),!e.objectStoreNames.contains("subtasks")){const i=e.createObjectStore("subtasks",{keyPath:"id"});i.createIndex("taskId","taskId",{unique:!1}),i.createIndex("status","status",{unique:!1})}e.objectStoreNames.contains("appState")||e.createObjectStore("appState",{keyPath:"key"})},s.onsuccess=a=>{$=a.target.result,t($)},s.onerror=a=>n(a.target.error)})}function b(t,n="readonly"){return $.transaction(t,n).objectStore(t)}function k(t){return new Promise((n,s)=>{t.onsuccess=()=>n(t.result),t.onerror=()=>s(t.error)})}async function R(t){return await g(),k(b("tasks","readwrite").put(t))}async function rt(t){return await g(),k(b("tasks").get(t))}async function B(){return await g(),k(b("tasks").getAll())}async function V(t){await g();const n=await T(t),s=b("subtasks","readwrite");for(const a of n)s.delete(a.id);return k(b("tasks","readwrite").delete(t))}async function O(t){return R(t)}async function E(t){return await g(),k(b("subtasks","readwrite").put(t))}async function ct(t){return await g(),k(b("subtasks").get(t))}async function T(t){await g();const n=b("subtasks").index("taskId");return k(n.getAll(t))}async function lt(t){return E(t)}async function P(t){return await g(),k(b("subtasks","readwrite").delete(t))}async function x(t){await g();const n=await k(b("appState").get(t));return n?n.value:null}async function I(t,n){return await g(),k(b("appState","readwrite").put({key:t,value:n}))}const j=Object.freeze(Object.defineProperty({__proto__:null,addSubtask:E,addTask:R,deleteSubtask:P,deleteTask:V,getAllTasks:B,getState:x,getSubtask:ct,getSubtasksByTask:T,getTask:rt,openDB:g,setState:I,updateSubtask:lt,updateTask:O},Symbol.toStringTag,{value:"Module"})),dt="modulepreload",ut=function(t){return"/focusflow/"+t},z={},H=function(n,s,a){let e=Promise.resolve();if(s&&s.length>0){let r=function(l){return Promise.all(l.map(d=>Promise.resolve(d).then(u=>({status:"fulfilled",value:u}),u=>({status:"rejected",reason:u}))))};document.getElementsByTagName("link");const c=document.querySelector("meta[property=csp-nonce]"),o=(c==null?void 0:c.nonce)||(c==null?void 0:c.getAttribute("nonce"));e=r(s.map(l=>{if(l=ut(l),l in z)return;z[l]=!0;const d=l.endsWith(".css"),u=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${u}`))return;const p=document.createElement("link");if(p.rel=d?"stylesheet":dt,d||(p.as="script"),p.crossOrigin="",p.href=l,o&&p.setAttribute("nonce",o),document.head.appendChild(p),d)return new Promise((m,h)=>{p.addEventListener("load",m),p.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${l}`)))})}))}function i(r){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=r,window.dispatchEvent(c),!c.defaultPrevented)throw r}return e.then(r=>{for(const c of r||[])c.status==="rejected"&&i(c.reason);return n().catch(i)})};let pt=0;function J(){return Date.now().toString(36)+"-"+(++pt).toString(36)+"-"+Math.random().toString(36).slice(2,7)}function mt(t,n,s=null){return{id:J(),title:t.trim(),category:n,intervalDays:s,createdAt:Date.now(),completedDates:[],lastScheduledDate:null,active:!0}}function Q(t,n,s,a,e=0){return{id:J(),taskId:t,title:n.trim(),estimatedMinutes:s,priority:a,order:e,status:"pending",completedAt:null,addedTimeMinutes:0}}const U={daily:t=>[{t:`Set up & prepare to start: ${t}`,min:5,p:3},{t:`Focus on the main work: ${t}`,min:20,p:5},{t:`Wrap up & quick review of: ${t}`,min:5,p:4}],periodic:t=>[{t:`Recall where you left off & review progress on: ${t}`,min:5,p:3},{t:`Do the core work session: ${t}`,min:20,p:5},{t:`Document progress & set reminders for next time: ${t}`,min:5,p:4}],oneoff:t=>[{t:`Research & figure out the approach for: ${t}`,min:10,p:3},{t:`Execute the main action: ${t}`,min:20,p:5},{t:`Verify completion & do any follow-ups for: ${t}`,min:10,p:4}],new:t=>[{t:`Understand requirements & ask clarifying questions about: ${t}`,min:10,p:3},{t:`Create first draft / initial attempt: ${t}`,min:20,p:5},{t:`Review your work & refine: ${t}`,min:10,p:4}]};function vt(t){return console.warn("Using fallback breakdown for:",t.title),(U[t.category]||U.daily)(t.title).map((a,e)=>Q(t.id,a.t,a.min,a.p,e))}async function C(t){var a;const n=await x("geminiApiKey");if(!n)return vt(t);const s=`
...
[
  ...
  `.trim();try{const e=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${n}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:s}]}],generationConfig:{temperature:.1}})});if(!e.ok){let o;try{o=await e.json()}catch{o={error:{message:`HTTP ${e.status}: ${e.statusText}`}}}const l=((a=o.error)==null?void 0:a.message)||`API Error: ${e.status}`;throw new Error(l)}let r=(await e.json()).candidates[0].content.parts[0].text;r=r.replace(/```json|```/g,"").trim();let c=JSON.parse(r);if(!Array.isArray(c)||c.length===0)throw new Error("Invalid format from API");return c.map((o,l)=>{const d=o.t||o.title||o.task||`Step ${l+1} for ${t.title}`,u=parseInt(o.min||o.minutes||o.time)||15,p=parseInt(o.p||o.priority)||3;return Q(t.id,d,u,p,l)})}catch(e){throw console.error("Gemini API Breakdown failed:",e),e}}function X(t){const n=Date.now(),s=new Date().getHours(),a=t.map(e=>{let i=0;if(i+=({new:5,oneoff:4,daily:3,periodic:2}[e.category]||2)*6,e.category==="periodic"&&e.intervalDays){const l=e.completedDates.length>0?new Date(e.completedDates[e.completedDates.length-1]).getTime():e.createdAt,u=(n-l)/(1e3*60*60*24)/e.intervalDays;i+=Math.min(u*12.5,25)}else if(e.category==="daily"){const l=new Date().toISOString().slice(0,10);e.completedDates.includes(l)||(i+=20)}else if(e.category==="new"){const l=(n-e.createdAt)/36e5;i+=Math.min(l*2,25)}s>=6&&s<=14&&(i+=10);const o=(n-e.createdAt)/(1e3*60*60*24);return i+=Math.max(10-o*2,0),{task:e,score:i}});return a.sort((e,i)=>i.score-e.score),a.map(e=>e.task)}function ft(t){return t.filter(s=>s.status!=="done").sort((s,a)=>s.order-a.order).slice(0,3)}function Z(){return new Date().toISOString().slice(0,10)}async function ht(){const t=await x("lastActiveDate"),n=Z();return t===n?!1:(await yt(n),await I("lastActiveDate",n),!0)}async function yt(t){const n=await B();for(const s of n)if(s.active)switch(s.category){case"daily":{const a=await T(s.id);for(const i of a)i.status==="done"?await P(i.id):await P(i.id);const e=C(s);for(const i of e)await E(i);break}case"periodic":{const a=s.completedDates.length>0?s.completedDates[s.completedDates.length-1]:null;let e=!1;if(!a)e=!0;else{const i=new Date(a);e=Math.floor((new Date(t)-i)/(1e3*60*60*24))>=(s.intervalDays||1)}if(e&&s.lastScheduledDate!==t){const i=await T(s.id);for(const c of i)await P(c.id);const r=C(s);for(const c of r)await E(c);s.lastScheduledDate=t,await O(s)}break}case"oneoff":{s.completedDates.includes(t)&&(s.active=!1,await O(s));break}case"new":{if((await T(s.id)).length===0){const e=C(s);for(const i of e)await E(i)}break}}}async function G(){const n=(await B()).filter(o=>o.active);if(n.length===0)return{subtasks:[],task:null};const s=X(n);let a=await x("currentFocusTaskIndex")||0;a>=s.length&&(a=0,await I("currentFocusTaskIndex",0));const e=s[a],i=await T(e.id);let r=await x(`focusBatch_${e.id}`),c=[];if(r&&Array.isArray(r)){const o=i.filter(d=>r.includes(d.id));!(o.length>0&&o.every(d=>d.status==="done"))&&o.length>0&&(c=o.sort((d,u)=>d.order-u.order))}if(c.length===0){const o=ft(i);if(o.length===0&&s.length>1)return await tt();o.length>0&&(c=o,await I(`focusBatch_${e.id}`,c.map(l=>l.id)))}return{subtasks:c,task:e}}async function tt(){const n=(await B()).filter(e=>e.active);if(n.length<=1)return await G();const s=X(n);let a=await x("currentFocusTaskIndex")||0;return a=(a+1)%s.length,await I("currentFocusTaskIndex",a),await G()}async function q(t){const{getSubtask:n,updateSubtask:s}=await H(async()=>{const{getSubtask:r,updateSubtask:c}=await Promise.resolve().then(()=>j);return{getSubtask:r,updateSubtask:c}},void 0),a=await n(t);if(!a)return;a.status="done",a.completedAt=Date.now(),await s(a);const i=(await T(a.taskId)).every(r=>r.status==="done");if(i){const{getTask:r,updateTask:c}=await H(async()=>{const{getTask:l,updateTask:d}=await Promise.resolve().then(()=>j);return{getTask:l,updateTask:d}},void 0),o=await r(a.taskId);o&&(o.completedDates.push(Z()),o.category==="oneoff"&&(o.active=!1),await c(o))}return i}const et={};let A=null;function M(t,n){et[t]=n}function gt(t){window.location.hash=t}function bt(t="#dashboard"){const n=()=>{const s=window.location.hash||t,a=et[s];a&&(A&&A.cleanup&&A.cleanup(),A=a())};window.addEventListener("hashchange",n),window.location.hash?n():window.location.hash=t}let _=!1;async function st(){return"Notification"in window?Notification.permission==="granted"?(_=!0,!0):Notification.permission!=="denied"?(_=await Notification.requestPermission()==="granted",_):!1:!1}function nt(t,n){_&&document.hidden&&new Notification(t,{body:n,icon:"/icons/icon-192.png",badge:"/icons/icon-192.png",vibrate:[200,100,200]}),f(t,n)}function f(t,n,s=4e3){const a=document.getElementById("toast-container");if(!a)return;const e=document.createElement("div");e.className="toast",e.innerHTML=`
    <div class="toast-title">${t}</div>
    ${n?`<div class="toast-body">${n}</div>`:""}
  `,a.appendChild(e),requestAnimationFrame(()=>e.classList.add("toast-show")),setTimeout(()=>{e.classList.remove("toast-show"),e.classList.add("toast-hide"),setTimeout(()=>e.remove(),400)},s)}function kt(t="Working...",n="Please wait while I process that."){let s=document.getElementById("global-loading-overlay");s||(s=document.createElement("div"),s.id="global-loading-overlay",s.className="global-loading-overlay",document.body.appendChild(s)),s.innerHTML=`
  ...
  `,requestAnimationFrame(()=>s.classList.add("show"))}function wt(){const t=document.getElementById("global-loading-overlay");t&&(t.classList.remove("show"),setTimeout(()=>{t.classList.contains("show")||(t.innerHTML="")},300))}const Tt=Object.freeze(Object.defineProperty({__proto__:null,hideGlobalLoading:wt,requestNotificationPermission:st,sendNotification:nt,showGlobalLoading:kt,showToast:f},Symbol.toStringTag,{value:"Module"}));let w=null,S=0,N=0;function St(t,n,s,a){S=n*60,N=n*60;const e=document.createElement("div");e.className="timer-overlay",e.id="timer-overlay",e.innerHTML=`
  ...
  `,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("timer-overlay-show"));let i=!1;const r=document.getElementById("timer-display"),c=document.getElementById("timer-progress-ring"),o=2*Math.PI*88,l=document.getElementById("btn-pause");function d(){r&&(r.textContent=Y(S));const y=1-S/N,v=o*y;c&&(c.style.strokeDashoffset=v)}function u(){i||(S--,d(),S<=0&&(clearInterval(w),w=null,p()))}function p(){nt("⏰ Time's Up!",`Timer for "${s}" has ended.`),"vibrate"in navigator&&navigator.vibrate([200,100,200,100,200]),document.querySelector(".timer-controls").style.display="none",document.getElementById("timeup-panel").style.display="block"}function m(y){S+=y*60,N+=y*60,document.getElementById("timeup-panel").style.display="none",document.querySelector(".timer-controls").style.display="flex",d(),w||(w=setInterval(u,1e3)),f("⏱️ Time Added",`+${y} minutes added`)}function h(y){w&&(clearInterval(w),w=null),e.classList.remove("timer-overlay-show"),e.classList.add("timer-overlay-hide"),setTimeout(()=>{e.remove(),a&&a()},400)}w=setInterval(u,1e3),l.addEventListener("click",()=>{i=!i,l.innerHTML=i?'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polygon points="5 3 19 12 5 21 5 3"/></svg> Resume':'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> Pause',l.classList.toggle("btn-resume",i)}),document.getElementById("btn-finish-early").addEventListener("click",async()=>{await q(t),f("✅ Done!","Great focus session!"),h()}),document.getElementById("btn-done-final").addEventListener("click",async()=>{await q(t),f("🎉 Complete!","You crushed it!"),h()}),e.querySelectorAll(".btn-add-time[data-minutes]").forEach(y=>{y.addEventListener("click",()=>{m(parseInt(y.dataset.minutes))})});const L=document.getElementById("random-slider"),K=document.getElementById("random-value");L.addEventListener("input",()=>{K.textContent=`${L.value} min`}),document.getElementById("btn-random-add").addEventListener("click",()=>{const y=parseInt(L.value),v=Math.max(1,Math.floor(Math.random()*y)+1);m(v),f("🎲 Random Time",`Added ${v} random minutes!`)})}function Y(t){const n=Math.floor(t/60),s=t%60;return`${n.toString().padStart(2,"0")}:${s.toString().padStart(2,"0")}`}let F=null;function xt(){const t=document.getElementById("app-content");return t.innerHTML=`
    <div class="dashboard">
  ...
  `,W(),D(),document.getElementById("btn-refresh").addEventListener("click",async()=>{const n=document.getElementById("btn-refresh");n.disabled=!0;const s=n.querySelector("svg");s.style.animation="spin 1s linear infinite",await tt(),await D(),n.disabled=!1,s.style.animation=""}),F=setInterval(W,6e4),{cleanup:()=>{F&&clearInterval(F)}}}function W(){const t=document.getElementById("greeting");if(!t)return;const n=new Date().getHours();let s="Good evening";n>=5&&n<12?s="Good morning":n>=12&&n<17&&(s="Good afternoon");const a=["🔥","💪","⚡","🚀","✨","🎯","💫"],e=a[Math.floor(Math.random()*a.length)];t.textContent=`${s} ${e}`}async function D(){const t=document.getElementById("focus-cards");if(!t)return;const{subtasks:n,task:s}=await G();if(!s||n.length===0){t.innerHTML=`
  ...
  `;c+=n.map((o,l)=>{const d=o.status==="done",u=o.priority>=4?"High":o.priority>=3?"Medium":"Low",p=o.priority>=4?"priority-high":o.priority>=3?"priority-med":"priority-low";return`
    ...
    `}).join(""),c+="</div>",t.innerHTML=c,t.querySelectorAll(".btn-start").forEach(o=>{o.addEventListener("click",l=>{const d=l.currentTarget.dataset.id,u=parseInt(l.currentTarget.dataset.minutes),p=n.find(m=>m.id===d);Et(d,u,p?p.title:"Task")})}),t.querySelectorAll(".btn-done").forEach(o=>{o.addEventListener("click",async l=>{const d=l.currentTarget.dataset.id,u=l.currentTarget.closest(".focus-card");u.classList.add("subtask-done-animation");const p=u.querySelector(".btn-done");p&&(p.disabled=!0);const m=await q(d),h=n.map(v=>v.id),{getSubtask:L}=await H(async()=>{const{getSubtask:v}=await Promise.resolve().then(()=>j);return{getSubtask:v}},void 0);if((await Promise.all(h.map(v=>L(v)))).every(v=>v&&v.status==="done"))setTimeout(()=>{It()},600);else if(m)f("🎉 Task Complete!","All subtasks for this task are done!"),D();else{u.classList.add("focus-card-done");const v=u.querySelector(".focus-card-actions");v&&(v.style.display="none"),f("✅ Step Complete!","Great work! Finish the set!")}})})}function It(){const t=document.createElement("div");t.className="batch-completion-overlay",t.innerHTML=`
    <div class="batch-completion-content">
      <div class="thumbs-up">👍</div>
      <h3>Batch Complete!</h3>
      <p>You're crushing it. Loading the next set...</p>
    </div>
  `,document.body.appendChild(t),setTimeout(()=>{t.classList.add("show")},10),setTimeout(()=>{t.classList.remove("show"),setTimeout(()=>{t.remove(),D()},400)},2500)}function Et(t,n,s){St(t,n,s,()=>{D()})}function Lt(){const t=document.getElementById("app-content");t.innerHTML=`
  ...
  `;let n="daily";const s=document.getElementById("add-task-form"),a=document.getElementById("interval-group"),e=document.getElementById("interval-days");return document.querySelectorAll(".category-btn").forEach(i=>{i.addEventListener("click",()=>{document.querySelectorAll(".category-btn").forEach(r=>r.classList.remove("active")),i.classList.add("active"),n=i.dataset.cat,a.style.display=n==="periodic"?"block":"none"})}),document.getElementById("interval-dec").addEventListener("click",()=>{e.value=Math.max(2,parseInt(e.value)-1)}),document.getElementById("interval-inc").addEventListener("click",()=>{e.value=Math.min(365,parseInt(e.value)+1)}),s.addEventListener("submit",async i=>{i.preventDefault();const r=document.getElementById("task-title").value.trim();if(!r)return;const c=n==="periodic"?parseInt(e.value):null,o=mt(r,n,c);await R(o);const l=document.getElementById("btn-submit"),d=l.innerHTML;l.innerHTML='<svg class="spin" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Breaking it down...',l.disabled=!0;const{showGlobalLoading:u,hideGlobalLoading:p}=await H(async()=>{const{showGlobalLoading:m,hideGlobalLoading:h}=await Promise.resolve().then(()=>Tt);return{showGlobalLoading:m,hideGlobalLoading:h}},void 0);u("AI Brainstorming...",`Breaking down "${r}" into actionable steps.`);try{const m=await C(o);for(const h of m)await E(h);p(),f("✅ Task Added!",`"${r}" has been broken into ${m.length} subtasks`),gt("#dashboard")}catch(m){console.error("Failed to break down:",m),p();try{await V(o.id)}catch(h){console.error("Failed to cleanup empty task:",h)}l.innerHTML=d,l.disabled=!1,f("❌ Error Breakdown",m.message||"Failed to break down task. Please check API Key or try again.")}}),{cleanup:()=>{}}}function $t(){const t=document.getElementById("app-content");return t.innerHTML=`
  ...
  `,at(),{cleanup:()=>{}}}async function at(){const t=document.getElementById("task-list-content");if(!t)return;const n=await B(),s=n.filter(r=>r.active);if(s.length===0){t.innerHTML=`
      <div class="empty-state">
        <div class="empty-icon">📋</div>
        <h3>No tasks yet</h3>
    ...
    `;for(const o of c){const l=await T(o.id),d=l.filter(h=>h.status==="done").length,u=l.length,p=u>0?Math.round(d/u*100):0;let m="";o.category==="periodic"&&o.intervalDays?m=`Every ${o.intervalDays} days`:o.completedDates.length>0&&(m=`Done ${o.completedDates.length} time${o.completedDates.length>1?"s":""}`),e+=`
    ...
    `),t.innerHTML=e,t.querySelectorAll(".btn-delete").forEach(r=>{r.addEventListener("click",async c=>{const o=c.currentTarget.dataset.taskId,l=c.currentTarget.closest(".task-item");confirm("Delete this task and all its subtasks?")&&(l.classList.add("card-exit"),setTimeout(async()=>{await V(o),f("🗑️ Deleted","Task removed"),at()},300))})})}function Dt(){const t=document.getElementById("app-content");t.innerHTML=`
  ...
  `;const n=document.getElementById("api-key-form"),s=document.getElementById("api-key");x("geminiApiKey").then(i=>{i&&(s.value=i)}),n.addEventListener("submit",async i=>{i.preventDefault();const r=s.value.trim();if(!r){await I("geminiApiKey",null),f("⚠️ Cleared","API Key removed");return}await I("geminiApiKey",r),f("✅ Saved!","Your API Key is securely stored.")});const a=document.getElementById("btn-test-api"),e=document.getElementById("api-test-results");return a.addEventListener("click",async()=>{var r;const i=s.value.trim();if(!i)return f("⚠️ Error","Please enter an API Key first.");a.disabled=!0,a.innerHTML="Testing...",e.style.display="block",e.innerHTML='<p class="loading-text">Fetching authorized models...</p>';try{const c=await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${i}`);if(!c.ok){const u=await c.json();throw new Error(((r=u.error)==null?void 0:r.message)||`HTTP ${c.status}`)}const l=(await c.json()).models||[];if(l.length===0){e.innerHTML='<p class="error-text">No models found for this key.</p>';return}const d=l.filter(u=>u.name.includes("gemini"));e.innerHTML=`
      ...
      `,f("✅ Connected",`Found ${d.length} Gemini models!`)}catch(c){console.error(c),e.innerHTML=`<p class="error-text">❌ Connection Failed: ${c.message}</p>`,f("❌ Failed","API Key check failed")}finally{a.disabled=!1,a.innerHTML="Test Connection"}}),{cleanup:()=>{}}}async function Bt(){const t=document.getElementById("app");t.innerHTML=`
    <div class="app-shell">
  ...
  `,await g(),await ht(),st(),M("#dashboard",xt),M("#add",Lt),M("#tasks",$t),M("#settings",Dt);const n=document.querySelectorAll(".nav-btn");function s(){const a=window.location.hash||"#dashboard";n.forEach(e=>{e.classList.toggle("active",e.dataset.route===a)})}n.forEach(a=>{a.addEventListener("click",()=>{window.location.hash=a.dataset.route})}),window.addEventListener("hashchange",s),bt("#dashboard"),s()}Bt().catch(console.error);
