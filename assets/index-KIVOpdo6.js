(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))a(e);new MutationObserver(e=>{for(const i of e)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function s(e){const i={};return e.integrity&&(i.integrity=e.integrity),e.referrerPolicy&&(i.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?i.credentials="include":e.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(e){if(e.ep)return;e.ep=!0;const i=s(e);fetch(e.href,i)}})();const st="focusflow",nt=1;let $=null;function y(){return new Promise((t,n)=>{if($)return t($);const s=indexedDB.open(st,nt);s.onupgradeneeded=a=>{const e=a.target.result;if(e.objectStoreNames.contains("tasks")||e.createObjectStore("tasks",{keyPath:"id"}).createIndex("category","category",{unique:!1}),!e.objectStoreNames.contains("subtasks")){const i=e.createObjectStore("subtasks",{keyPath:"id"});i.createIndex("taskId","taskId",{unique:!1}),i.createIndex("status","status",{unique:!1})}e.objectStoreNames.contains("appState")||e.createObjectStore("appState",{keyPath:"key"})},s.onsuccess=a=>{$=a.target.result,t($)},s.onerror=a=>n(a.target.error)})}function h(t,n="readonly"){return $.transaction(t,n).objectStore(t)}function b(t){return new Promise((n,s)=>{t.onsuccess=()=>n(t.result),t.onerror=()=>s(t.error)})}async function G(t){return await y(),b(h("tasks","readwrite").put(t))}async function at(t){return await y(),b(h("tasks").get(t))}async function L(){return await y(),b(h("tasks").getAll())}async function R(t){await y();const n=await w(t),s=h("subtasks","readwrite");for(const a of n)s.delete(a.id);return b(h("tasks","readwrite").delete(t))}async function F(t){return G(t)}async function x(t){return await y(),b(h("subtasks","readwrite").put(t))}async function it(t){return await y(),b(h("subtasks").get(t))}async function w(t){await y();const n=h("subtasks").index("taskId");return b(n.getAll(t))}async function ot(t){return x(t)}async function M(t){return await y(),b(h("subtasks","readwrite").delete(t))}async function E(t){await y();const n=await b(h("appState").get(t));return n?n.value:null}async function I(t,n){return await y(),b(h("appState","readwrite").put({key:t,value:n}))}const _=Object.freeze(Object.defineProperty({__proto__:null,addSubtask:x,addTask:G,deleteSubtask:M,deleteTask:R,getAllTasks:L,getState:E,getSubtask:it,getSubtasksByTask:w,getTask:at,openDB:y,setState:I,updateSubtask:ot,updateTask:F},Symbol.toStringTag,{value:"Module"})),rt="modulepreload",ct=function(t){return"/focusflow/"+t},K={},V=function(n,s,a){let e=Promise.resolve();if(s&&s.length>0){let r=function(l){return Promise.all(l.map(u=>Promise.resolve(u).then(d=>({status:"fulfilled",value:d}),d=>({status:"rejected",reason:d}))))};document.getElementsByTagName("link");const c=document.querySelector("meta[property=csp-nonce]"),o=(c==null?void 0:c.nonce)||(c==null?void 0:c.getAttribute("nonce"));e=r(s.map(l=>{if(l=ct(l),l in K)return;K[l]=!0;const u=l.endsWith(".css"),d=u?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${d}`))return;const p=document.createElement("link");if(p.rel=u?"stylesheet":rt,u||(p.as="script"),p.crossOrigin="",p.href=l,o&&p.setAttribute("nonce",o),document.head.appendChild(p),u)return new Promise((f,m)=>{p.addEventListener("load",f),p.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${l}`)))})}))}function i(r){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=r,window.dispatchEvent(c),!c.defaultPrevented)throw r}return e.then(r=>{for(const c of r||[])c.status==="rejected"&&i(c.reason);return n().catch(i)})};let lt=0;function W(){return Date.now().toString(36)+"-"+(++lt).toString(36)+"-"+Math.random().toString(36).slice(2,7)}function dt(t,n,s=null){return{id:W(),title:t.trim(),category:n,intervalDays:s,createdAt:Date.now(),completedDates:[],lastScheduledDate:null,active:!0}}function J(t,n,s,a,e=0){return{id:W(),taskId:t,title:n.trim(),estimatedMinutes:s,priority:a,order:e,status:"pending",completedAt:null,addedTimeMinutes:0}}const z={daily:t=>[{t:`Set up & prepare to start: ${t}`,min:5,p:3},{t:`Focus on the main work: ${t}`,min:20,p:5},{t:`Wrap up & quick review of: ${t}`,min:5,p:4}],periodic:t=>[{t:`Recall where you left off & review progress on: ${t}`,min:5,p:3},{t:`Do the core work session: ${t}`,min:20,p:5},{t:`Document progress & set reminders for next time: ${t}`,min:5,p:4}],oneoff:t=>[{t:`Research & figure out the approach for: ${t}`,min:10,p:3},{t:`Execute the main action: ${t}`,min:20,p:5},{t:`Verify completion & do any follow-ups for: ${t}`,min:10,p:4}],new:t=>[{t:`Understand requirements & ask clarifying questions about: ${t}`,min:10,p:3},{t:`Create first draft / initial attempt: ${t}`,min:20,p:5},{t:`Review your work & refine: ${t}`,min:10,p:4}]};function ut(t){return console.warn("Using fallback breakdown for:",t.title),(z[t.category]||z.daily)(t.title).map((a,e)=>J(t.id,a.t,a.min,a.p,e))}async function C(t){var a;const n=await E("geminiApiKey");if(!n)return ut(t);const s=`
...
[
  ...
  `.trim();try{const e=await fetch(`https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash:generateContent?key=${n}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:s}]}],generationConfig:{temperature:.1}})});if(!e.ok){let o;try{o=await e.json()}catch{o={error:{message:`HTTP ${e.status}: ${e.statusText}`}}}const l=((a=o.error)==null?void 0:a.message)||`API Error: ${e.status}`;throw new Error(l)}let r=(await e.json()).candidates[0].content.parts[0].text;r=r.replace(/```json|```/g,"").trim();let c=JSON.parse(r);if(!Array.isArray(c)||c.length===0)throw new Error("Invalid format from API");return c.map((o,l)=>{const u=o.t||o.title||o.task||`Step ${l+1} for ${t.title}`,d=parseInt(o.min||o.minutes||o.time)||15,p=parseInt(o.p||o.priority)||3;return J(t.id,u,d,p,l)})}catch(e){throw console.error("Gemini API Breakdown failed:",e),e}}function pt(t){const n=Date.now(),s=new Date().getHours(),a=t.map(e=>{let i=0;if(i+=({new:5,oneoff:4,daily:3,periodic:2}[e.category]||2)*6,e.category==="periodic"&&e.intervalDays){const l=e.completedDates.length>0?new Date(e.completedDates[e.completedDates.length-1]).getTime():e.createdAt,d=(n-l)/(1e3*60*60*24)/e.intervalDays;i+=Math.min(d*12.5,25)}else if(e.category==="daily"){const l=new Date().toISOString().slice(0,10);e.completedDates.includes(l)||(i+=20)}else if(e.category==="new"){const l=(n-e.createdAt)/36e5;i+=Math.min(l*2,25)}s>=6&&s<=14&&(i+=10);const o=(n-e.createdAt)/(1e3*60*60*24);return i+=Math.max(10-o*2,0),{task:e,score:i}});return a.sort((e,i)=>i.score-e.score),a.map(e=>e.task)}function mt(t){return t.filter(s=>s.status!=="done").sort((s,a)=>s.order-a.order).slice(0,3)}function Q(){return new Date().toISOString().slice(0,10)}async function vt(){const t=await E("lastActiveDate"),n=Q();return t===n?!1:(await ft(n),await I("lastActiveDate",n),!0)}async function ft(t){const n=await L();for(const s of n)if(s.active)switch(s.category){case"daily":{const a=await w(s.id);for(const i of a)i.status==="done"?await M(i.id):await M(i.id);const e=C(s);for(const i of e)await x(i);break}case"periodic":{const a=s.completedDates.length>0?s.completedDates[s.completedDates.length-1]:null;let e=!1;if(!a)e=!0;else{const i=new Date(a);e=Math.floor((new Date(t)-i)/(1e3*60*60*24))>=(s.intervalDays||1)}if(e&&s.lastScheduledDate!==t){const i=await w(s.id);for(const c of i)await M(c.id);const r=C(s);for(const c of r)await x(c);s.lastScheduledDate=t,await F(s)}break}case"oneoff":{s.completedDates.includes(t)&&(s.active=!1,await F(s));break}case"new":{if((await w(s.id)).length===0){const e=C(s);for(const i of e)await x(i)}break}}}async function O(){const n=(await L()).filter(c=>c.active);if(n.length===0)return{subtasks:[],task:null};const s=pt(n);let a=await E("currentFocusTaskIndex")||0;a>=s.length&&(a=0,await I("currentFocusTaskIndex",0));const e=s[a],i=await w(e.id),r=mt(i);return r.length===0&&s.length>1?await X():{subtasks:r,task:e}}async function X(){const n=(await L()).filter(a=>a.active);if(n.length<=1)return await O();let s=await E("currentFocusTaskIndex")||0;return s=(s+1)%n.length,await I("currentFocusTaskIndex",s),await O()}async function q(t){const{getSubtask:n,updateSubtask:s}=await V(async()=>{const{getSubtask:r,updateSubtask:c}=await Promise.resolve().then(()=>_);return{getSubtask:r,updateSubtask:c}},void 0),a=await n(t);if(!a)return;a.status="done",a.completedAt=Date.now(),await s(a);const i=(await w(a.taskId)).every(r=>r.status==="done");if(i){const{getTask:r,updateTask:c}=await V(async()=>{const{getTask:l,updateTask:u}=await Promise.resolve().then(()=>_);return{getTask:l,updateTask:u}},void 0),o=await r(a.taskId);o&&(o.completedDates.push(Q()),o.category==="oneoff"&&(o.active=!1),await c(o))}return i}const Z={};let B=null;function A(t,n){Z[t]=n}function yt(t){window.location.hash=t}function gt(t="#dashboard"){const n=()=>{const s=window.location.hash||t,a=Z[s];a&&(B&&B.cleanup&&B.cleanup(),B=a())};window.addEventListener("hashchange",n),window.location.hash?n():window.location.hash=t}let P=!1;async function ht(){return"Notification"in window?Notification.permission==="granted"?(P=!0,!0):Notification.permission!=="denied"?(P=await Notification.requestPermission()==="granted",P):!1:!1}function bt(t,n){P&&document.hidden&&new Notification(t,{body:n,icon:"/icons/icon-192.png",badge:"/icons/icon-192.png",vibrate:[200,100,200]}),v(t,n)}function v(t,n,s=4e3){const a=document.getElementById("toast-container");if(!a)return;const e=document.createElement("div");e.className="toast",e.innerHTML=`
    <div class="toast-title">${t}</div>
    ${n?`<div class="toast-body">${n}</div>`:""}
  `,a.appendChild(e),requestAnimationFrame(()=>e.classList.add("toast-show")),setTimeout(()=>{e.classList.remove("toast-show"),e.classList.add("toast-hide"),setTimeout(()=>e.remove(),400)},s)}let k=null,T=0,N=0;function kt(t,n,s,a){T=n*60,N=n*60;const e=document.createElement("div");e.className="timer-overlay",e.id="timer-overlay",e.innerHTML=`
  ...
  `,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("timer-overlay-show"));let i=!1;const r=document.getElementById("timer-display"),c=document.getElementById("timer-progress-ring"),o=2*Math.PI*88,l=document.getElementById("btn-pause");function u(){r&&(r.textContent=U(T));const g=1-T/N,D=o*g;c&&(c.style.strokeDashoffset=D)}function d(){i||(T--,u(),T<=0&&(clearInterval(k),k=null,p()))}function p(){bt("⏰ Time's Up!",`Timer for "${s}" has ended.`),"vibrate"in navigator&&navigator.vibrate([200,100,200,100,200]),document.querySelector(".timer-controls").style.display="none",document.getElementById("timeup-panel").style.display="block"}function f(g){T+=g*60,N+=g*60,document.getElementById("timeup-panel").style.display="none",document.querySelector(".timer-controls").style.display="flex",u(),k||(k=setInterval(d,1e3)),v("⏱️ Time Added",`+${g} minutes added`)}function m(g){k&&(clearInterval(k),k=null),e.classList.remove("timer-overlay-show"),e.classList.add("timer-overlay-hide"),setTimeout(()=>{e.remove(),a&&a()},400)}k=setInterval(d,1e3),l.addEventListener("click",()=>{i=!i,l.innerHTML=i?'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polygon points="5 3 19 12 5 21 5 3"/></svg> Resume':'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> Pause',l.classList.toggle("btn-resume",i)}),document.getElementById("btn-finish-early").addEventListener("click",async()=>{await q(t),v("✅ Done!","Great focus session!"),m()}),document.getElementById("btn-done-final").addEventListener("click",async()=>{await q(t),v("🎉 Complete!","You crushed it!"),m()}),e.querySelectorAll(".btn-add-time[data-minutes]").forEach(g=>{g.addEventListener("click",()=>{f(parseInt(g.dataset.minutes))})});const S=document.getElementById("random-slider"),et=document.getElementById("random-value");S.addEventListener("input",()=>{et.textContent=`${S.value} min`}),document.getElementById("btn-random-add").addEventListener("click",()=>{const g=parseInt(S.value),D=Math.max(1,Math.floor(Math.random()*g)+1);f(D),v("🎲 Random Time",`Added ${D} random minutes!`)})}function U(t){const n=Math.floor(t/60),s=t%60;return`${n.toString().padStart(2,"0")}:${s.toString().padStart(2,"0")}`}let j=null;function wt(){const t=document.getElementById("app-content");return t.innerHTML=`
    <div class="dashboard">
  ...
  `,Y(),H(),document.getElementById("btn-refresh").addEventListener("click",async()=>{const n=document.getElementById("btn-refresh");n.disabled=!0;const s=n.querySelector("svg");s.style.animation="spin 1s linear infinite",await X(),await H(),n.disabled=!1,s.style.animation=""}),j=setInterval(Y,6e4),{cleanup:()=>{j&&clearInterval(j)}}}function Y(){const t=document.getElementById("greeting");if(!t)return;const n=new Date().getHours();let s="Good evening";n>=5&&n<12?s="Good morning":n>=12&&n<17&&(s="Good afternoon");const a=["🔥","💪","⚡","🚀","✨","🎯","💫"],e=a[Math.floor(Math.random()*a.length)];t.textContent=`${s} ${e}`}async function H(){const t=document.getElementById("focus-cards");if(!t)return;const{subtasks:n,task:s}=await O();if(!s||n.length===0){t.innerHTML=`
  ...
  `;c+=n.map((o,l)=>{const u=o.priority>=4?"High":o.priority>=3?"Medium":"Low",d=o.priority>=4?"priority-high":o.priority>=3?"priority-med":"priority-low";return`
      <div class="focus-card card-enter" style="animation-delay: ${l*.1}s" data-subtask-id="${o.id}">
        <div class="focus-card-header">
          <span class="step-badge">Step ${o.order+1}</span>
          <span class="priority-badge ${d}">${u}</span>
        </div>
    ...
    `}).join(""),c+="</div>",t.innerHTML=c,t.querySelectorAll(".btn-start").forEach(o=>{o.addEventListener("click",l=>{const u=l.currentTarget.dataset.id,d=parseInt(l.currentTarget.dataset.minutes),p=n.find(f=>f.id===u);Tt(u,d,p?p.title:"Task")})}),t.querySelectorAll(".btn-done").forEach(o=>{o.addEventListener("click",async l=>{const u=l.currentTarget.dataset.id;l.currentTarget.closest(".focus-card").classList.add("card-complete"),setTimeout(async()=>{await q(u)?v("🎉 Task Complete!","All subtasks for this task are done!"):v("✅ Subtask Done!","Great work! Keep going!"),H()},500)})})}function Tt(t,n,s){kt(t,n,s,()=>{H()})}function St(){const t=document.getElementById("app-content");t.innerHTML=`
  ...
  `;let n="daily";const s=document.getElementById("add-task-form"),a=document.getElementById("interval-group"),e=document.getElementById("interval-days");return document.querySelectorAll(".category-btn").forEach(i=>{i.addEventListener("click",()=>{document.querySelectorAll(".category-btn").forEach(r=>r.classList.remove("active")),i.classList.add("active"),n=i.dataset.cat,a.style.display=n==="periodic"?"block":"none"})}),document.getElementById("interval-dec").addEventListener("click",()=>{e.value=Math.max(2,parseInt(e.value)-1)}),document.getElementById("interval-inc").addEventListener("click",()=>{e.value=Math.min(365,parseInt(e.value)+1)}),s.addEventListener("submit",async i=>{i.preventDefault();const r=document.getElementById("task-title").value.trim();if(!r)return;const c=n==="periodic"?parseInt(e.value):null,o=dt(r,n,c);await G(o);const l=document.getElementById("btn-submit"),u=l.innerHTML;l.innerHTML='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Breaking it down...',l.disabled=!0,l.classList.add("loading");try{const d=await C(o);for(const m of d)await x(m);s.style.display="none";const p=document.getElementById("breakdown-preview");p.style.display="block";const f=document.getElementById("subtask-list");f.innerHTML=d.map((m,S)=>`
      ...
      `).join(""),v("✅ Task Added!",`"${r}" has been broken into ${d.length} subtasks`)}catch(d){console.error("Failed to break down:",d);try{await R(o.id)}catch(p){console.error("Failed to cleanup empty task:",p)}l.innerHTML=u,l.disabled=!1,l.classList.remove("loading"),v("❌ Error Breakdown",d.message||"Failed to break down task. Please check API Key or try again.")}document.getElementById("btn-go-dashboard").addEventListener("click",()=>{yt("#dashboard")})}),{cleanup:()=>{}}}function xt(){const t=document.getElementById("app-content");return t.innerHTML=`
  ...
  `,tt(),{cleanup:()=>{}}}async function tt(){const t=document.getElementById("task-list-content");if(!t)return;const n=await L(),s=n.filter(r=>r.active);if(s.length===0){t.innerHTML=`
      <div class="empty-state">
        <div class="empty-icon">📋</div>
        <h3>No tasks yet</h3>
    ...
    `;for(const o of c){const l=await w(o.id),u=l.filter(m=>m.status==="done").length,d=l.length,p=d>0?Math.round(u/d*100):0;let f="";o.category==="periodic"&&o.intervalDays?f=`Every ${o.intervalDays} days`:o.completedDates.length>0&&(f=`Done ${o.completedDates.length} time${o.completedDates.length>1?"s":""}`),e+=`
  ...
  `;const n=document.getElementById("api-key-form"),s=document.getElementById("api-key");E("geminiApiKey").then(i=>{i&&(s.value=i)}),n.addEventListener("submit",async i=>{i.preventDefault();const r=s.value.trim();if(!r){await I("geminiApiKey",null),v("⚠️ Cleared","API Key removed");return}await I("geminiApiKey",r),v("✅ Saved!","Your API Key is securely stored.")});const a=document.getElementById("btn-test-api"),e=document.getElementById("api-test-results");return a.addEventListener("click",async()=>{var r;const i=s.value.trim();if(!i)return v("⚠️ Error","Please enter an API Key first.");a.disabled=!0,a.innerHTML="Testing...",e.style.display="block",e.innerHTML='<p class="loading-text">Fetching authorized models...</p>';try{const c=await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${i}`);if(!c.ok){const d=await c.json();throw new Error(((r=d.error)==null?void 0:r.message)||`HTTP ${c.status}`)}const l=(await c.json()).models||[];if(l.length===0){e.innerHTML='<p class="error-text">No models found for this key.</p>';return}const u=l.filter(d=>d.name.includes("gemini"));e.innerHTML=`
      ...
      `,v("✅ Connected",`Found ${u.length} Gemini models!`)}catch(c){console.error(c),e.innerHTML=`<p class="error-text">❌ Connection Failed: ${c.message}</p>`,v("❌ Failed","API Key check failed")}finally{a.disabled=!1,a.innerHTML="Test Connection"}}),{cleanup:()=>{}}}async function Et(){const t=document.getElementById("app");t.innerHTML=`
    <div class="app-shell">
  ...
  `,await y(),await vt(),ht(),A("#dashboard",wt),A("#add",St),A("#tasks",xt),A("#settings",It);const n=document.querySelectorAll(".nav-btn");function s(){const a=window.location.hash||"#dashboard";n.forEach(e=>{e.classList.toggle("active",e.dataset.route===a)})}n.forEach(a=>{a.addEventListener("click",()=>{window.location.hash=a.dataset.route})}),window.addEventListener("hashchange",s),gt("#dashboard"),s()}Et().catch(console.error);
