...
[
  ...
  `,dt(),{cleanup:()=>{}}}async function dt(){const t=document.getElementById("task-list-content");if(!t)return;const s=await A(),n=s.filter(i=>i.active);if(n.length===0){t.innerHTML=`
    ...
    `;for(const r of c){const p=await S(r.id),v=p.filter(h=>h.status==="done").length,m=p.length,f=m>0?Math.round(v/m*100):0;let T="";r.category==="periodic"&&r.intervalDays?T=`Every ${r.intervalDays} days`:r.completedDates.length>0&&(T=`Done ${r.completedDates.length} time${r.completedDates.length>1?"s":""}`),e+=`
        <div class="task-item card-enter" data-task-id="${r.id}">
          <div class="task-item-click-area">
            <div class="task-item-content">
              <div class="task-item-title" id="title-${r.id}">
                ${f===100?'<span class="congratulation-symbol">🏆</span>':""}
                ${r.title}
              </div>
              ${T?`<div class="task-item-meta">${T}</div>`:""}
              <div class="task-item-progress">
                <div class="progress-bar">
                  <div class="progress-fill" id="progress-${r.id}" style="width: ${f}%"></div>
                </div>
          ...
          <div class="subtasks-expand-container" id="subs-container-${r.id}">
            ...
            <div class="subtask-expansion-footer">
              <button class="btn btn-primary btn-focus" data-task-id="${r.id}" ${f===100?"disabled":""}>
                 ...
                 ${f===100?"Task Complete":"Go to Focus"}
              </button>
            </div>
          </div>
  ...
  `,await g(),await rt(),Et(),C("#dashboard",Bt),C("#add",Pt),C("#tasks",Ct),C("#settings",_t);const s=document.querySelectorAll(".nav-btn");function n(){const a=window.location.hash||"#dashboard";s.forEach(e=>{e.classList.toggle("active",e.dataset.route===a)})}s.forEach(a=>{a.addEventListener("click",()=>{window.location.hash=a.dataset.route})}),window.addEventListener("hashchange",n),lt("#dashboard"),n()}Ft().catch(console.error);
