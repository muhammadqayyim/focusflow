...
[
  ...
  `,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("timer-overlay-show"));let i=!1;const c=document.getElementById("timer-display"),d=document.getElementById("timer-progress-ring"),o=2*Math.PI*88,r=document.getElementById("btn-pause");function l(){c&&(c.textContent=U(T));const h=1-T/O,L=o*h;d&&(d.style.strokeDashoffset=L)}function u(){i||(T--,l(),T<=0&&(clearInterval(k),k=null,p()))}function p(){bt("⏰ Time's Up!",`Timer for "${s}" has ended.`),"vibrate"in navigator&&navigator.vibrate([200,100,200,100,200]),document.querySelector(".timer-controls").style.display="none",document.getElementById("timeup-panel").style.display="block"}function v(h){T+=h*60,O+=h*60,document.getElementById("timeup-panel").style.display="none",document.querySelector(".timer-controls").style.display="flex",l(),k||(k=setInterval(u,1e3)),f("⏱️ Time Added",`+${h} minutes added`)}function m(h){k&&(clearInterval(k),k=null),e.classList.remove("timer-overlay-show"),e.classList.add("timer-overlay-hide"),setTimeout(()=>{e.remove(),a&&a()},400)}k=setInterval(u,1e3),r.addEventListener("click",()=>{i=!i,r.innerHTML=i?'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polygon points="5 3 19 12 5 21 5 3"/></svg> Resume':'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> Pause',r.classList.toggle("btn-resume",i)}),document.getElementById("btn-finish-early").addEventListener("click",async()=>{await q(t),f("✅ Done!","Great focus session!"),m()}),document.getElementById("btn-done-final").addEventListener("click",async()=>{await q(t),f("🎉 Complete!","You crushed it!"),m()}),e.querySelectorAll(".btn-add-time[data-minutes]").forEach(h=>{h.addEventListener("click",()=>{v(parseInt(h.dataset.minutes))})});const S=document.getElementById("random-slider"),et=document.getElementById("random-value");S.addEventListener("input",()=>{et.textContent=`${S.value} min`}),document.getElementById("btn-random-add").addEventListener("click",()=>{const h=parseInt(S.value),L=Math.max(1,Math.floor(Math.random()*h)+1);v(L),f("🎲 Random Time",`Added ${L} random minutes!`)})}function U(t){const n=Math.floor(t/60),s=t%60;return`${n.toString().padStart(2,"0")}:${s.toString().padStart(2,"0")}`}let j=null;function wt(){const t=document.getElementById("app-content");return t.innerHTML=`
    <div class="dashboard">
  ...
  `;d+=n.map((o,r)=>{const l=o.priority>=4?"High":o.priority>=3?"Medium":"Low",u=o.priority>=4?"priority-high":o.priority>=3?"priority-med":"priority-low";return`
      <div class="focus-card card-enter" style="animation-delay: ${r*.1}s" data-subtask-id="${o.id}">
    ...
    `}).join(""),d+="</div>",t.innerHTML=d,t.querySelectorAll(".btn-start").forEach(o=>{o.addEventListener("click",r=>{const l=r.currentTarget.dataset.id,u=parseInt(r.currentTarget.dataset.minutes),p=n.find(v=>v.id===l);Tt(l,u,p?p.title:"Task")})}),t.querySelectorAll(".btn-done").forEach(o=>{o.addEventListener("click",async r=>{const l=r.currentTarget.dataset.id;r.currentTarget.closest(".focus-card").classList.add("card-complete"),setTimeout(async()=>{await q(l)?f("🎉 Task Complete!","All subtasks for this task are done!"):f("✅ Subtask Done!","Great work! Keep going!"),N()},500)})})}function Tt(t,n,s){kt(t,n,s,()=>{N()})}function St(){const t=document.getElementById("app-content");t.innerHTML=`
  ...
  `;let n="daily";const s=document.getElementById("add-task-form"),a=document.getElementById("interval-group"),e=document.getElementById("interval-days");return document.querySelectorAll(".category-btn").forEach(i=>{i.addEventListener("click",()=>{document.querySelectorAll(".category-btn").forEach(c=>c.classList.remove("active")),i.classList.add("active"),n=i.dataset.cat,a.style.display=n==="periodic"?"block":"none"})}),document.getElementById("interval-dec").addEventListener("click",()=>{e.value=Math.max(2,parseInt(e.value)-1)}),document.getElementById("interval-inc").addEventListener("click",()=>{e.value=Math.min(365,parseInt(e.value)+1)}),s.addEventListener("submit",async i=>{i.preventDefault();const c=document.getElementById("task-title").value.trim();if(!c)return;const d=n==="periodic"?parseInt(e.value):null,o=lt(c,n,d);await R(o);const r=document.getElementById("btn-submit"),l=r.innerHTML;r.innerHTML='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Breaking it down...',r.disabled=!0,r.classList.add("loading");try{const u=await P(o);for(const m of u)await x(m);s.style.display="none";const p=document.getElementById("breakdown-preview");p.style.display="block";const v=document.getElementById("subtask-list");v.innerHTML=u.map((m,S)=>`
  ...
  `,tt(),{cleanup:()=>{}}}async function tt(){const t=document.getElementById("task-list-content");if(!t)return;const n=await $(),s=n.filter(c=>c.active);if(s.length===0){t.innerHTML=`
      <div class="empty-state">
        <div class="empty-icon">📋</div>
        <h3>No tasks yet</h3>
    ...
    `;return}const a=[{key:"daily",label:"🔄 Daily Tasks",cls:"cat-daily"},{key:"periodic",label:"📅 Periodic Tasks",cls:"cat-periodic"},{key:"oneoff",label:"🎯 One-off Tasks",cls:"cat-oneoff"},{key:"new",label:"⭐ New Tasks",cls:"cat-new"}];let e="";for(const c of a){const d=s.filter(o=>o.category===c.key);if(d.length!==0){e+=`
      <div class="task-category-section">
        <div class="category-header ${c.cls}">
          <span>${c.label}</span>
          <span class="task-count">${d.length}</span>
        </div>
        <div class="task-items">
    `;for(const o of d){const r=await w(o.id),l=r.filter(m=>m.status==="done").length,u=r.length,p=u>0?Math.round(l/u*100):0;let v="";o.category==="periodic"&&o.intervalDays?v=`Every ${o.intervalDays} days`:o.completedDates.length>0&&(v=`Done ${o.completedDates.length} time${o.completedDates.length>1?"s":""}`),e+=`
      ...
      `}e+="</div></div>"}}const i=n.filter(c=>!c.active);i.length>0&&(e+=`
      <div class="task-category-section">
    ...
    `),t.innerHTML=e,t.querySelectorAll(".btn-delete").forEach(c=>{c.addEventListener("click",async d=>{const o=d.currentTarget.dataset.taskId,r=d.currentTarget.closest(".task-item");confirm("Delete this task and all its subtasks?")&&(r.classList.add("card-exit"),setTimeout(async()=>{await _(o),f("🗑️ Deleted","Task removed"),tt()},300))})})}function It(){const t=document.getElementById("app-content");t.innerHTML=`
