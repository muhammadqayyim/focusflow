(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))a(e);new MutationObserver(e=>{for(const i of e)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function n(e){const i={};return e.integrity&&(i.integrity=e.integrity),e.referrerPolicy&&(i.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?i.credentials="include":e.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(e){if(e.ep)return;e.ep=!0;const i=n(e);fetch(e.href,i)}})();const nt="focusflow",st=1;let D=null;function y(){return new Promise((t,s)=>{if(D)return t(D);const n=indexedDB.open(nt,st);n.onupgradeneeded=a=>{const e=a.target.result;if(e.objectStoreNames.contains("tasks")||e.createObjectStore("tasks",{keyPath:"id"}).createIndex("category","category",{unique:!1}),!e.objectStoreNames.contains("subtasks")){const i=e.createObjectStore("subtasks",{keyPath:"id"});i.createIndex("taskId","taskId",{unique:!1}),i.createIndex("status","status",{unique:!1})}e.objectStoreNames.contains("appState")||e.createObjectStore("appState",{keyPath:"key"})},n.onsuccess=a=>{D=a.target.result,t(D)},n.onerror=a=>s(a.target.error)})}function g(t,s="readonly"){return D.transaction(t,s).objectStore(t)}function b(t){return new Promise((s,n)=>{t.onsuccess=()=>s(t.result),t.onerror=()=>n(t.error)})}async function R(t){return await y(),b(g("tasks","readwrite").put(t))}async function at(t){return await y(),b(g("tasks").get(t))}async function L(){return await y(),b(g("tasks").getAll())}async function Y(t){await y();const s=await w(t),n=g("subtasks","readwrite");for(const a of s)n.delete(a.id);return b(g("tasks","readwrite").delete(t))}async function H(t){return R(t)}async function x(t){return await y(),b(g("subtasks","readwrite").put(t))}async function it(t){return await y(),b(g("subtasks").get(t))}async function w(t){await y();const s=g("subtasks").index("taskId");return b(s.getAll(t))}async function ot(t){return x(t)}async function M(t){return await y(),b(g("subtasks","readwrite").delete(t))}async function E(t){await y();const s=await b(g("appState").get(t));return s?s.value:null}async function I(t,s){return await y(),b(g("appState","readwrite").put({key:t,value:s}))}const _=Object.freeze(Object.defineProperty({__proto__:null,addSubtask:x,addTask:R,deleteSubtask:M,deleteTask:Y,getAllTasks:L,getState:E,getSubtask:it,getSubtasksByTask:w,getTask:at,openDB:y,setState:I,updateSubtask:ot,updateTask:H},Symbol.toStringTag,{value:"Module"})),rt="modulepreload",ct=function(t){return"/focusflow/"+t},G={},V=function(s,n,a){let e=Promise.resolve();if(n&&n.length>0){let o=function(d){return Promise.all(d.map(u=>Promise.resolve(u).then(l=>({status:"fulfilled",value:l}),l=>({status:"rejected",reason:l}))))};document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),c=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));e=o(n.map(d=>{if(d=ct(d),d in G)return;G[d]=!0;const u=d.endsWith(".css"),l=u?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${l}`))return;const p=document.createElement("link");if(p.rel=u?"stylesheet":rt,u||(p.as="script"),p.crossOrigin="",p.href=d,c&&p.setAttribute("nonce",c),document.head.appendChild(p),u)return new Promise((v,m)=>{p.addEventListener("load",v),p.addEventListener("error",()=>m(new Error(`Unable to preload CSS for ${d}`)))})}))}function i(o){const r=new Event("vite:preloadError",{cancelable:!0});if(r.payload=o,window.dispatchEvent(r),!r.defaultPrevented)throw o}return e.then(o=>{for(const r of o||[])r.status==="rejected"&&i(r.reason);return s().catch(i)})};let dt=0;function W(){return Date.now().toString(36)+"-"+(++dt).toString(36)+"-"+Math.random().toString(36).slice(2,7)}function lt(t,s,n=null){return{id:W(),title:t.trim(),category:s,intervalDays:n,createdAt:Date.now(),completedDates:[],lastScheduledDate:null,active:!0}}function J(t,s,n,a,e=0){return{id:W(),taskId:t,title:s.trim(),estimatedMinutes:n,priority:a,order:e,status:"pending",completedAt:null,addedTimeMinutes:0}}const K={daily:t=>[{t:`Set up & prepare to start: ${t}`,min:5,p:3},{t:`Focus on the main work: ${t}`,min:20,p:5},{t:`Wrap up & quick review of: ${t}`,min:5,p:4}],periodic:t=>[{t:`Recall where you left off & review progress on: ${t}`,min:5,p:3},{t:`Do the core work session: ${t}`,min:20,p:5},{t:`Document progress & set reminders for next time: ${t}`,min:5,p:4}],oneoff:t=>[{t:`Research & figure out the approach for: ${t}`,min:10,p:3},{t:`Execute the main action: ${t}`,min:20,p:5},{t:`Verify completion & do any follow-ups for: ${t}`,min:10,p:4}],new:t=>[{t:`Understand requirements & ask clarifying questions about: ${t}`,min:10,p:3},{t:`Create first draft / initial attempt: ${t}`,min:20,p:5},{t:`Review your work & refine: ${t}`,min:10,p:4}]};function ut(t){return console.warn("Using fallback breakdown for:",t.title),(K[t.category]||K.daily)(t.title).map((a,e)=>J(t.id,a.t,a.min,a.p,e))}async function C(t){const s=await E("geminiApiKey");if(!s)return ut(t);const n=`
...
[
  ...
  `,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add("timer-overlay-show"));let i=!1;const o=document.getElementById("timer-display"),r=document.getElementById("timer-progress-ring"),c=2*Math.PI*88,d=document.getElementById("btn-pause");function u(){o&&(o.textContent=z(T));const h=1-T/O,$=c*h;r&&(r.style.strokeDashoffset=$)}function l(){i||(T--,u(),T<=0&&(clearInterval(k),k=null,p()))}function p(){bt("⏰ Time's Up!",`Timer for "${n}" has ended.`),"vibrate"in navigator&&navigator.vibrate([200,100,200,100,200]),document.querySelector(".timer-controls").style.display="none",document.getElementById("timeup-panel").style.display="block"}function v(h){T+=h*60,O+=h*60,document.getElementById("timeup-panel").style.display="none",document.querySelector(".timer-controls").style.display="flex",u(),k||(k=setInterval(l,1e3)),f("⏱️ Time Added",`+${h} minutes added`)}function m(h){k&&(clearInterval(k),k=null),e.classList.remove("timer-overlay-show"),e.classList.add("timer-overlay-hide"),setTimeout(()=>{e.remove(),a&&a()},400)}k=setInterval(l,1e3),d.addEventListener("click",()=>{i=!i,d.innerHTML=i?'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polygon points="5 3 19 12 5 21 5 3"/></svg> Resume':'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> Pause',d.classList.toggle("btn-resume",i)}),document.getElementById("btn-finish-early").addEventListener("click",async()=>{await q(t),f("✅ Done!","Great focus session!"),m()}),document.getElementById("btn-done-final").addEventListener("click",async()=>{await q(t),f("🎉 Complete!","You crushed it!"),m()}),e.querySelectorAll(".btn-add-time[data-minutes]").forEach(h=>{h.addEventListener("click",()=>{v(parseInt(h.dataset.minutes))})});const S=document.getElementById("random-slider"),et=document.getElementById("random-value");S.addEventListener("input",()=>{et.textContent=`${S.value} min`}),document.getElementById("btn-random-add").addEventListener("click",()=>{const h=parseInt(S.value),$=Math.max(1,Math.floor(Math.random()*h)+1);v($),f("🎲 Random Time",`Added ${$} random minutes!`)})}function z(t){const s=Math.floor(t/60),n=t%60;return`${s.toString().padStart(2,"0")}:${n.toString().padStart(2,"0")}`}let j=null;function wt(){const t=document.getElementById("app-content");return t.innerHTML=`
    <div class="dashboard">
  ...
  `,U(),N(),document.getElementById("btn-refresh").addEventListener("click",async()=>{const s=document.getElementById("btn-refresh");s.disabled=!0;const n=s.querySelector("svg");n.style.animation="spin 1s linear infinite",await X(),await N(),s.disabled=!1,n.style.animation=""}),j=setInterval(U,6e4),{cleanup:()=>{j&&clearInterval(j)}}}function U(){const t=document.getElementById("greeting");if(!t)return;const s=new Date().getHours();let n="Good evening";s>=5&&s<12?n="Good morning":s>=12&&s<17&&(n="Good afternoon");const a=["🔥","💪","⚡","🚀","✨","🎯","💫"],e=a[Math.floor(Math.random()*a.length)];t.textContent=`${n} ${e}`}async function N(){const t=document.getElementById("focus-cards");if(!t)return;const{subtasks:s,task:n}=await F();if(!n||s.length===0){t.innerHTML=`
  ...
  `;r+=s.map((c,d)=>{const u=c.priority>=4?"High":c.priority>=3?"Medium":"Low",l=c.priority>=4?"priority-high":c.priority>=3?"priority-med":"priority-low";return`
    ...
    `}).join(""),r+="</div>",t.innerHTML=r,t.querySelectorAll(".btn-start").forEach(c=>{c.addEventListener("click",d=>{const u=d.currentTarget.dataset.id,l=parseInt(d.currentTarget.dataset.minutes),p=s.find(v=>v.id===u);Tt(u,l,p?p.title:"Task")})}),t.querySelectorAll(".btn-done").forEach(c=>{c.addEventListener("click",async d=>{const u=d.currentTarget.dataset.id;d.currentTarget.closest(".focus-card").classList.add("card-complete"),setTimeout(async()=>{await q(u)?f("🎉 Task Complete!","All subtasks for this task are done!"):f("✅ Subtask Done!","Great work! Keep going!"),N()},500)})})}function Tt(t,s,n){kt(t,s,n,()=>{N()})}function St(){const t=document.getElementById("app-content");t.innerHTML=`
  ...
  `;let s="daily";const n=document.getElementById("add-task-form"),a=document.getElementById("interval-group"),e=document.getElementById("interval-days");return document.querySelectorAll(".category-btn").forEach(i=>{i.addEventListener("click",()=>{document.querySelectorAll(".category-btn").forEach(o=>o.classList.remove("active")),i.classList.add("active"),s=i.dataset.cat,a.style.display=s==="periodic"?"block":"none"})}),document.getElementById("interval-dec").addEventListener("click",()=>{e.value=Math.max(2,parseInt(e.value)-1)}),document.getElementById("interval-inc").addEventListener("click",()=>{e.value=Math.min(365,parseInt(e.value)+1)}),n.addEventListener("submit",async i=>{i.preventDefault();const o=document.getElementById("task-title").value.trim();if(!o)return;const r=s==="periodic"?parseInt(e.value):null,c=lt(o,s,r);await R(c);const d=document.getElementById("btn-submit"),u=d.innerHTML;d.innerHTML='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Breaking it down...',d.disabled=!0,d.classList.add("loading");try{const l=await C(c);for(const m of l)await x(m);n.style.display="none";const p=document.getElementById("breakdown-preview");p.style.display="block";const v=document.getElementById("subtask-list");v.innerHTML=l.map((m,S)=>`
  ...
  `,tt(),{cleanup:()=>{}}}async function tt(){const t=document.getElementById("task-list-content");if(!t)return;const s=await L(),n=s.filter(o=>o.active);if(n.length===0){t.innerHTML=`
      <div class="empty-state">
        <div class="empty-icon">📋</div>
        <h3>No tasks yet</h3>
    ...
    `;for(const c of r){const d=await w(c.id),u=d.filter(m=>m.status==="done").length,l=d.length,p=l>0?Math.round(u/l*100):0;let v="";c.category==="periodic"&&c.intervalDays?v=`Every ${c.intervalDays} days`:c.completedDates.length>0&&(v=`Done ${c.completedDates.length} time${c.completedDates.length>1?"s":""}`),e+=`
        <div class="task-item card-enter" data-task-id="${c.id}">
          <div class="task-item-content">
            ...
            <div class="task-item-progress">
              ...
              <span class="progress-text">${u}/${l} subtasks</span>
            </div>
          </div>
  ...
  `;const s=document.getElementById("api-key-form"),n=document.getElementById("api-key");return E("geminiApiKey").then(a=>{a&&(n.value=a)}),s.addEventListener("submit",async a=>{a.preventDefault();const e=n.value.trim();if(!e){await I("geminiApiKey",null),f("⚠️ Cleared","API Key removed");return}await I("geminiApiKey",e),f("✅ Saved!","Your API Key is securely stored.")}),{cleanup:()=>{}}}async function Et(){const t=document.getElementById("app");t.innerHTML=`
    <div class="app-shell">
  ...
  `,await y(),await vt(),gt(),A("#dashboard",wt),A("#add",St),A("#tasks",xt),A("#settings",It);const s=document.querySelectorAll(".nav-btn");function n(){const a=window.location.hash||"#dashboard";s.forEach(e=>{e.classList.toggle("active",e.dataset.route===a)})}s.forEach(a=>{a.addEventListener("click",()=>{window.location.hash=a.dataset.route})}),window.addEventListener("hashchange",n),ht("#dashboard"),n()}Et().catch(console.error);
